...
- `VERIFIED_IN_CODE` / `RISK`: Email, Internal, and Phone article backends inherit the wrapped MIMEBase create path; the separate Chat backend has its own wrapped lifecycle. `D724TicketAudit 0.8.17` wraps the Invalid fallback backend's unknown-channel metadata delete route with a scope/audit transaction and normalized deletion event. Candidate runtime regression is still required before that route is accepted.
...
- `VERIFIED_BY_CURRENT_TEST` (static only): `D724TicketAudit 0.8.17` removes stale `0.8.10`/`0.8.12` status-version literals. The status command now reports its module version through one `$VERSION` value, its regression expects the current package version, and `Test-Foundation.ps1` rejects future manifest/status/test drift. Runtime command execution remains part of the current candidate rerun.
