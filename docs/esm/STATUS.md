...
- `VERIFIED_IN_CODE` / `RISK`: Email, Internal, and Phone article backends inherit the wrapped MIMEBase create path; the separate Chat backend has its own wrapped lifecycle. `D724TicketAudit 0.8.18` retains the Invalid fallback backend's unknown-channel metadata delete route with a scope/audit transaction and normalized deletion event. Candidate runtime regression is still required before that route is accepted.
...
- `VERIFIED_BY_CURRENT_TEST` (static only): `D724TicketAudit 0.8.18` keeps the status command, module versions, regression, and manifest aligned; `Test-Foundation.ps1` rejects future drift. Runtime command execution remains part of the current candidate rerun.
- `VERIFIED_IN_CODE` / `TEST_EXISTS_NOT_RERUN`: direct core comparison found `TicketArchiveFlagSet` missing from the earlier business-mutation inventory. `D724TicketAudit 0.8.18` now wraps it through the scope-lock/audit transaction contract. `TicketAudit.t` requires audit-disabled flag and scope rollback, recovered persistence, one scope-version advance, and exactly one normalized `ticket.archive_flag.updated` event. Foundation locks the contract; current candidate MariaDB execution is still required. Personal watcher/seen flags, accounted-time writes, escalation-index maintenance, and article-storage switching are not claimed covered.
- `RISK` / `SMARTER_MODEL_DECISION_REQUIRED`: source comparison also found the unwrapped direct write `TicketUnlockTimeoutUpdate`. MIMEBase and Chat article creation invoke it inside their audited parent transaction without checking its return value. A naive nested wrapper can silently roll back only the timeout while the article continues, or advance the scope and make the parent article compare-and-swap fail against its stale version. No code change was made with this unresolved ownership contract. The decision record and required direct/MIMEBase/Chat rollback matrix are in [Ticket audit coverage inventory](security/TICKET-AUDIT-COVERAGE.md); continue this item with a higher-reasoning model before implementation.
