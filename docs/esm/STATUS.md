...
- `VERIFIED_BY_CURRENT_TEST`: `D724TicketAudit 0.8.9` wraps Generic Interface `TicketUpdate::Run` in a request-level transaction and gives nested ticket mutations savepoints. Candidate MariaDB regression passed `TicketAudit.t` (128) and `TicketAuditStatus.t` (11), including a later request failure that rolls an earlier successful title mutation, scope version, audit record, and cache state back. Artifact: `/home/test/careoncloud-releases/20260725/.codex-backup-p0-api-20260802/ticket-audit-0.8.9-gi-atomicity.log`. `PARTIAL`: this is operation-adapter evidence; an authenticated transport contract with a real multi-field request is still required.
- `IMPLEMENTED_NOT_YET_VERIFIED`: `development/d724/Test-CleanPackageLifecycle.ps1` now installs `D724TicketAudit` into its isolated clean database and invokes `Accept-GenericInterfaceTicketUpdate.pl`. That acceptance creates a one-use REST provider, rotates the fresh `admin` password to a random process-scoped value, posts a real authenticated two-field `TicketUpdate` request to `/nph-genericinterface.pl`, and verifies persisted title/priority, tenant scope version, normalized audit events, chain validity, and provider deletion. No GitHub Actions run or successful standalone candidate result is claimed yet. A 2026-08-02 SSH wrapper investigation did establish the fresh candidate web health transition, but its post-`quick_setup` command chain did not reach package execution; it is not transport acceptance evidence.
