...
- `VERIFIED_IN_CODE` / `RISK`: GenericAgent's `AutoPriorityIncrease` path reaches the audited `TicketPrioritySet` wrapper, but the scheduler worker executes GenericAgent jobs as `UserID => 1` without a D724 tenant automation context. The ticket-level mutation therefore has audit coverage, while job selection remains capable of using an unrestricted administrator context. It is explicitly not tenant-isolation, atomic-request, or daemon-acceptance evidence; a tenant-scoped adapter and clean candidate cross-tenant regression remain P0 work.
