...
- `VERIFIED_IN_CODE` / `TEST_EXISTS_NOT_RERUN`: `Generate-SourceArtifact.ps1` refuses a dirty worktree, creates a commit-bound CareOnCloud source ZIP, and writes a SHA-256 manifest with the exact commit and D724 package version inventory. Its contract test verifies the archive hash and current commit; the Foundation workflow uploads both as a release-evidence artifact. This is provenance evidence, not a signed artifact or reproducible-image proof.
