...
- `RISK` / authenticated brand candidate: the isolated fresh `d724/esm:dev` image (`sha256:f65ec8a0160d08c501bd1d3d22228b485c467231c9acda8c8fa58b773ecbd813`) reached `quick_setup.pl`, but setup still emitted `/otobo/index.pl` URLs and the web container did not return to its health check after the required restart. Therefore authenticated CareOnCloud login/dashboard branding is **not accepted**. Evidence: `/home/test/careoncloud-releases/20260725/.codex-backup-p0-api-20260802/authenticated-brand-acceptance.log`. The exact temporary Compose project, network, and volumes were removed; the active `d724-esm` containers remained healthy. The next candidate must capture the failing web logs and correct the runtime/bootstrap path before repeating this acceptance.
