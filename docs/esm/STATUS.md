...
- `VERIFIED_IN_CODE` / `TEST_EXISTS_NOT_RERUN`: the Foundation workflow adds a Trivy filesystem vulnerability gate for fixed HIGH/CRITICAL library findings and uploads its JSON result even when the gate fails. This covers repository dependency metadata; it does not scan the resolved CareOnCloud image, OS packages, or base-image digest, so image vulnerability scanning remains open.
...
- `VERIFIED_IN_CODE` / `RISK`: Email, Internal, and Phone article backends inherit the wrapped MIMEBase create path; the separate Chat backend has its own wrapped lifecycle. The `Invalid` fallback backend can delete unknown-channel metadata through an unwrapped route and remains explicitly outside accepted audit coverage.
