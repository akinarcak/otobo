...
- `VERIFIED_IN_CODE` / `RISK`: Email, Internal, and Phone article backends inherit the wrapped MIMEBase create path; the separate Chat backend has its own wrapped lifecycle. `D724TicketAudit 0.8.15` wraps the Invalid fallback backend's unknown-channel metadata delete route with a scope/audit transaction and normalized deletion event. Candidate runtime regression is still required before that route is accepted.
