...
`CareOnCloudCommitment 0.5.0`, SLA/OLA escalation outbox'indaki webhook aksiyonlarini
...
`CareOnCloudWebhook 0.3.0` ayni teslimat cekirdeginin uzerine genel lifecycle
...
`CareOnCloud::Commitment::WebhookEndpoints` hash'ine `endpoint::URL` ve
...
Tarayici immutable `careoncloud_audit_event` zincirini tenant + subscription cursor'u
ile sirali okur. Eslesen olay `(tenant, subscription, audit sequence)` icin
ortak `careoncloud_escalation_outbox` tablosuna tek kayit yazar; outbox yazimi ve cursor
...
```text
X-CareOnCloud-Signature-Version: v1
X-CareOnCloud-Signature-Timestamp: 2026-07-27 12:34:56
X-CareOnCloud-Delivery-ID: 9002
X-CareOnCloud-Tenant: tenant-key
X-CareOnCloud-Signature-256: sha256=<64 lowercase hex>
```

`X-CareOnCloud-Signature-Timestamp` UTC'dir. Imzalanan byte dizisi asagidaki gibi
...
```text
bin/careoncloud.Console.pl Admin::CareOnCloud::CommitmentReplay \
...
`Admin::CareOnCloud::CommitmentStatus --json` pending/retry/processing/delivered/dead
sayaclarini, webhook delivered/dead sayaclarini, lifetime attempt ve replay
toplamlarini raporlar.

`Admin::CareOnCloud::WebhookStatus --json`, genel lifecycle abonelik outbox'u icin
...
`development/careoncloud/Accept-CommitmentReplay.pl` ile `careoncloud-demo` uzerinde gercek
...
outbox `74` ile tek kez teslim edildi. Son paketlerle tum CareOnCloud regresyonu 42
