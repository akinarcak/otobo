...
- [x] `SLA-01a`: tenant-safe resolution commitment policy/instance/event modeli, CareOnCloud ESM business calendar hesabi, pause/resume, warning/breach sweep ve request lifecycle entegrasyonu (`2026-07-24`).
...
- [x] `AUD-01b-ticket-core`: CareOnCloud ESM ticket create ile title/queue/customer/lock/state/owner/responsible/priority ve DB-backed MIME article create icin immutable tenant scope, transaction-atomic audit ve legacy migration kapisi (`2026-07-24`).
- [x] `SEC-01b-ticket`: CareOnCloud ESM TicketSearch sorgu-oncesi tenant filtresi, raw bypass reddi, immutable-scope tekil okuma ve Generic Interface ortak get/history/update tenant kapisi (`2026-07-24`).
...
- [x] `SEC-03b-pkce`: CareOnCloud ESM OAuth2 authorization URL ve token exchange katmanlarında RFC 7636 S256 challenge/verifier taşıma ve fail-closed doğrulama (`2026-07-25`).
...
1. `AUD-01b-core`: kapsanan direct ticket mutatorlari, Chat lifecycle, Generic Interface `TicketCreate`/`TicketUpdate` ve core pending-check scheduler yolundan sonra; `TicketCreate` candidate MariaDB kabul tekrarını, GenericAgent/diger scheduler-daemon yolları, non-MIME article backend'leri, harici eklenti/dogrudan DB yazimlari, cross-system index/storage side effect'leri ile retention/legal hold ve WORM sink icin audit/outbox completeness
