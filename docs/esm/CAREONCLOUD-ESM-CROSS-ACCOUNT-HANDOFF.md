...
`D:\Projects\CareOnCloud ESM\docs\esm\CAREONCLOUD-ESM-AI-MASTER-CONTEXT.md`
...
- Yerel proje kökü: `D:\Data\Documents\OneDrive - Data Market Bilgi Hizmetleri A.S\Documents\CareOnCloud ESM`
- Master context: `D:\Projects\CareOnCloud ESM\docs\esm\CAREONCLOUD-ESM-AI-MASTER-CONTEXT.md`
...
- Tamamlanan son P0 hedefi: CareOnCloudTicketAudit cache/merge audit ownership düzeltmesi, quick-setup legacy fixture temizliği ve izole aday runtime kabulü.
- Kanıt: CareOnCloudTicketAudit 4 dosya / 227 test, CareOnCloudAPI 6 dosya / 160 test ve CareOnCloudReporting 4 dosya / 73 test güncel adayda `PASS`; `Test-Foundation.ps1` mevcut kaynakta `PASS`. Aktif `careoncloud-esm-*` servislerine dokunulmadı.
...
- `Test-Foundation.ps1` 18 CareOnCloud paket manifesti ve dosya listesini geçti.
- CI’de repository dependency Trivy taraması ve temiz lifecycle sonrası `careoncloud/esm:dev` OS/library image taraması tanımlı.
- Temiz lifecycle ve yerel `Build`/`Up` yolları `docker buildx version` önkoşulu ile korunuyor.
- Test sunucusunda Docker Compose 2.40.3 ve kullanıcı-local Docker Buildx 0.34.1 mevcut; güncel aday image üretildi. Eski `careoncloud/esm:dev` image’ı kullanıcı onayıyla kaldırıldı; aktif servisler korunuyor.
- Aktif eski servisler korunmalıdır: `careoncloud-esm-web-1`, `careoncloud-esm-daemon-1`, `careoncloud-esm-db-1`, `careoncloud-esm-redis-1`, `careoncloud-esm-elastic-1`.
- Bu nedenle aşağıdaki iddialar runtime kabulü değildir: eski `careoncloud/esm:dev` imajı, eski volume’lar, tarihsel MariaDB test logları veya yalnız Perl syntax kontrolü.
...
4. `powershell -ExecutionPolicy Bypass -File .\development\careoncloud\Test-Foundation.ps1` çalıştır.
...
- CareOnCloud paket manifestleri, SOPM sürümleri ve dependency sırasını denetlemek.
...
5. Buildx varsa ayrı Compose project/volume/port ile aday lifecycle çalıştır; aktif `careoncloud-esm-*` projesine dokunma.
...
- Aktif `careoncloud-esm-*` konteynerlerini restart/stop/recreate etme.
...
- Eski `careoncloud/esm:dev` imajını CareOnCloud runtime kanıtı olarak kullanma.
