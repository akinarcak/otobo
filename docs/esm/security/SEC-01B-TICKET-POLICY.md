...
`CareOnCloudTicketAudit 0.7.1`, CareOnCloud ESM'nun resmi `Ticket::CustomModule` uzatma noktasinda merkezi `CareOnCloud::TicketPolicy` servisini yukler.

- Agent baglami aktif `careoncloud_tenant_agent_role` kayitlarindan, customer baglami aktif customer-company tenant'inden uretilir.
...
- Tekil erisim immutable `careoncloud_ticket_scope` ve aktif tenant kaydindan dogrulanir; ticket'in degisebilir UI alanlarina guvenilmez.
...
- Platform bypass yalniz mevcut `CareOnCloud::TenantGuard::AllowPlatformAdmin` emergency ayari ve acik `platform_admin` baglami ile mumkundur.
...
- Tum CareOnCloud regresyonu: 26 dosya / 486 test `PASS`.
- Kalici demo kabul betigi: `development/careoncloud/Accept-TicketPolicy.pl` agent directory baglamindan `CareOnCloudAUD20260724001` kaydini arar, scope tenant'ini dogrular, raw bypass'i reddeder ve Generic Interface ortak kontrolunu calistirir.
- Test sunucusu kabul sonucu: `demo.agent` / UserID `47`, tenant `careoncloud-demo`, gorunen ticket listesi `[9]`, raw bypass denied ve Generic Interface access `1`.
- OPM SHA-256: `a574c4e22fef7520c7d86a7ab418f13243ae8e8963e742150b1c6d097e3d9e29`.

## Acik kapsam

Generic Interface için OAuth client kimliği, rate limit, idempotency ve sürümlü `/api/v1` kontratı `API-01` kapsamındadır. Daemon actor bağlamı `SEC-01B-DAEMON.md`, operasyon raporu `REPORT-01.md`, cache namespace'i `SEC-01B-CACHE.md` ve aktif Elasticsearch runtime'i `SEC-01B-SEARCH.md` ile tamamlanmıştır. Operasyon-bazlı role/action matrisi `CareOnCloudTenantGuard 0.8.0` ve `CareOnCloudTicketAudit 0.8.1` ile kapatılmıştır.
