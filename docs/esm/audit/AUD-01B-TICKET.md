...
`CareOnCloudTicketAudit 0.7.1`, CareOnCloud ESM'nun resmi `Ticket::CustomModule` extension mekanizmasini kullanir; upstream `Kernel::System::Ticket` dosyasi degistirilmez. Su yazimlar kapsanir:
...
Her ticket `careoncloud_ticket_scope` tablosunda degismez tenant kimligi ve monoton mutation version'i alir. Yeni ticket aktif CareOnCloud tenant'a eslesen `CustomerID/CustomerNo` olmadan yaratilamaz. Customer user ayni tenant icinde degisebilir; CustomerID'nin baska tenant'a tasinmasi reddedilir.
...
- `Admin::CareOnCloud::TicketScopeBackfill`: CustomerID'si aktif CareOnCloud tenant'a zaten eslesen unbound ticket'lari sinirli ve atomik batch ile scope'a alir; her kayit `ticket.scope.backfilled` olayi uretir.
- `Admin::CareOnCloud::TicketScopeAssign`: tek bir uyumsuz legacy ticket icin acik tenant secimi ister. CustomerID farkliysa ayrica `--replace-customer-id` ve `--confirm` olmadan yazmaz; replacement+scope+audit ayni transaction'dir.
- `Admin::CareOnCloud::TicketAuditStatus --json`: core/scoped/unbound/backfillable/invalid sayilarini ve en fazla 100 unbound kimligi raporlar. Her unbound ticket health sonucunu basarisiz yapar.

## Kanit

- Paket: 2 dosya / 67 test `PASS`.
- Tum CareOnCloud regresyonu: 25 dosya / 469 test `PASS`.
...
- Demo kabul: `CareOnCloudAUD20260724001`, TicketID `9`, state `open`, scope version `3`, actions `ticket.created`, `ticket.state.updated`, `ticket.article.created`, chain valid. Ikinci calisma `Created=0`.
- Legacy kabul: kurulum ticket'i ID `1` acik replacement onayiyla `careoncloud-demo` tenant'ina atandi; `CoreTickets=2`, `Tickets=2`, `UnboundTickets=0`, `InvalidTenantTickets=0`.
