...
Durum: `AUD-01a` tamamlandi (`2026-07-24`). `AUD-01b` acik.
...
Bu katman kaydedilmis olaylarin sonradan sessizce degistirilmesini algilar. Mevcut surum, uygulama mutasyonu ile audit append'inin her adapter'da atomik oldugunu veya harici bir saldirgan karsisinda DB-disinda immutable saklama yapildigini iddia etmez.
...
`D724Request 0.3.2` su olaylari senkron olarak uretir:
...
- Demo tenant zinciri kabul sonunda `Valid=1`, 11 event.

## AUD-01b acik kapsam

- Uygulama mutasyonu + audit append icin transaction veya transactional outbox completeness garantisi.
- OTOBO ticket/article, Generic Interface, catalog yonetimi, commitment/escalation, kimlik ve konfigurasyon adapter'lari.
