...
# The stage builder-for-kerberos is only needed for careoncloud-nginx-kerberos
#
...
ENV CAREONCLOUD_WEB_HTTPS_PORT=443
...
# CAREONCLOUD_NGINX_SSL_CERTIFICATE and CAREONCLOUD_NGINX_SSL_CERTIFICATE_KEY. Values for these variable must
...
FROM base AS careoncloud-nginx-webproxy

# Actually there are two config templates in the directory 'templates'. One for plain Nginx and one for Nginx with
# Kerberos support. The not needed template is moved out of the way.
RUN mv templates/careoncloud_nginx-kerberos.conf.template templates/careoncloud_nginx-kerberos.conf.template.hidden

LABEL org.opencontainers.image.title='CareOnCloud ESM nginx'
...
FROM base AS careoncloud-nginx-kerberos-webproxy
...
# Actually there are two config templates in the directory 'templates'. One for plain Nginx and one for Nginx with
# Kerberos support. The not needed template is moved out of the way.
RUN mv templates/careoncloud_nginx.conf.template templates/careoncloud_nginx.conf.template.hidden
...
LABEL org.opencontainers.image.title='CareOnCloud ESM nginx Kerberos SSO'
