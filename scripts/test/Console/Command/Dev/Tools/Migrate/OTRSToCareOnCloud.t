...
my $CommandObject = $Kernel::OM->Get('Kernel::System::Console::Command::Dev::Tools::Migrate::OTRSToCareOnCloud');
...
subtest
    ...
    sub {
        ...
        my $TestDir   = dir($Home)->subdir('tmp/Test/Migrate/OTRSToCareOnCloud');
        ...
        my $SampleFile = dir($Home)->file('scripts/test/sample/SysConfig/MigrateOTRSToCareOnCloud.xml');
        my $SourceFile = $SampleFile->copy_to( $SourceDir->file('MigrateOTRSToCareOnCloud.xml') );
        my $TargetFile = $TargetDir->file('MigrateOTRSToCareOnCloud.xml');
        ...
        my $ExpectedResultFile = dir($Home)->file('scripts/test/sample/SysConfig/MigrateOTRSToCareOnCloudResult.xml');
        is(
            [ $TargetFile->slurp() ],
