...
ok( $DB->Do( SQL => 'UPDATE d724_tenant SET status = ? WHERE key_name = ?', Bind => [ \$Inactive, \$TenantA ] ), 'tenant A deactivated' );
...
$Helper->ConfigSettingChange( Key => 'D724::TenantCache::Enabled', Value => 0 );
