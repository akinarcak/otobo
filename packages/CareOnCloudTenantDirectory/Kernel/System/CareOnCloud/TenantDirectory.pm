...
package Kernel::System::CareOnCloud::TenantDirectory;
...
sub _TenantUpdate {
    ...
    my $Success = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE careoncloud_tenant SET name = ?, status = ?, version = version + 1, '
...
sub _MembershipRevoke {
    ...
    if ( $Param{Role} eq 'tenant_admin' ) {
        ...
        my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
        $DBObject->Prepare(
            SQL => "SELECT status FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = ? AND role_name = 'tenant_admin'",
            Bind => [ \$TenantID, \$MemberUserID ], Limit => 1,
        );
        my ($TargetStatus) = $DBObject->FetchrowArray();
        if ( ( $TargetStatus // q{} ) eq 'active' ) {
            $DBObject->Prepare(
                SQL => "SELECT COUNT(*) FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND role_name = 'tenant_admin' AND status = 'active'",
                Bind => [ \$TenantID ],
            );
    ...
    if ( $Before->{Status} ne 'revoked' ) {
        ...
        my @Bind = map { \$_ } @Values;
        my $Success = $Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => "UPDATE careoncloud_tenant_agent_role SET status = 'revoked', version = version + 1, change_time = current_timestamp, change_by = ? "
...
sub MembershipList {
    ...
    $DBObject->Prepare(
        SQL => 'SELECT user_id, role_name, status, version, create_time, create_by, change_time, change_by '
            . 'FROM careoncloud_tenant_agent_role WHERE tenant_id = ? ORDER BY user_id, role_name',
        Bind => [ \$TenantID ],
    );
    my @Data;
...
sub ContextGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => "SELECT r.tenant_id, r.role_name FROM careoncloud_tenant_agent_role r "
            . "INNER JOIN careoncloud_tenant t ON t.key_name = r.tenant_id "
...
sub _MembershipUpsert {
    ...
    $DBObject->Prepare(
        SQL => 'SELECT id, status, version FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = ? AND role_name = ?',
    ...
    if ($ID) {
        if ( $Status ne 'active' ) {
            ...
            my @Bind = map { \$_ } @Values;
            $Success = $DBObject->Do(
                SQL => "UPDATE careoncloud_tenant_agent_role SET status = 'active', version = version + 1, change_time = current_timestamp, change_by = ? WHERE id = ? AND version = ?",
                Bind => \@Bind,
            );
            $Version++;
        }
        ...
        my @Bind = map { \$_ } @Values;
        $Success = $DBObject->Do(
            SQL => 'INSERT INTO careoncloud_tenant_agent_role '
...
sub _TenantLock {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id FROM careoncloud_tenant WHERE key_name = ? FOR UPDATE',
        Bind => [ \$TenantID ],
    );
...
sub _MembershipRowGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT status, version FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = ? AND role_name = ?',
        Bind => [ \$TenantID, \$MemberUserID, \$Role ], Limit => 1,
    );
...
sub _TenantRowGet {
    ...
    $DBObject->Prepare(
        SQL => 'SELECT id, key_name, name, status, version, create_time, create_by, change_time, change_by '
            . 'FROM careoncloud_tenant WHERE key_name = ?', Bind => [ \$TenantID ], Limit => 1,
    );
