...
$DB->Prepare(
    SQL => 'SELECT COUNT(*) FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = 1 AND role_name = ?',
    Bind => [ \$Tenant, \$Role ],
);
...
$DB->Prepare(
    SQL => 'SELECT status, version FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = 1 AND role_name = ?',
...
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_event WHERE tenant_id = ?', Bind => [ \$Tenant ] ), 'audit events are cleaned up' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_head WHERE tenant_id = ?', Bind => [ \$Tenant ] ), 'audit head is cleaned up' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_tenant_agent_role WHERE tenant_id = ?', Bind => [ \$Tenant ] ), 'membership fixture is cleaned up' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_tenant WHERE key_name = ?', Bind => [ \$Tenant ] ), 'tenant fixture is cleaned up' );
done_testing;
