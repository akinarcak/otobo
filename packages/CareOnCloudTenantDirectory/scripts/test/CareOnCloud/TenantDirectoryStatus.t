...
my $Command = $Kernel::OM->Get('Kernel::System::Console::Command::Admin::CareOnCloud::TenantDirectoryStatus');
...
is( $Status->{Package}, 'CareOnCloudTenantDirectory', 'status identifies package' );
is( $Status->{Version}, '0.2.1', 'status identifies version' );
is( [ sort keys %{ $Status->{Tables} } ], [qw(careoncloud_tenant careoncloud_tenant_agent_role)], 'both directory tables reported' );
