...
# --
package Kernel::System::CareOnCloud::Webhook;
...
our @ObjectDependencies = (
    ...
    'Kernel::System::CareOnCloud::EscalationDispatcher',
...
sub _SubscriptionCreate {
    ...
    $DB->Prepare(
        SQL => 'SELECT id FROM careoncloud_webhook_subscription WHERE tenant_id = ? AND key_name = ?',
    ...
    return $Self->_Error('DATABASE_ERROR') if !$DB->Do(
        SQL => 'INSERT INTO careoncloud_webhook_subscription (tenant_id, key_name, name, endpoint_key, event_patterns_json, status, cursor_sequence, version, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, ?, ?, ?, ?, 1, current_timestamp, ?, current_timestamp, ?)',
        Bind => \@Bind,
    );
...
sub _SubscriptionUpdate {
    ...
    return $Self->_Error('DATABASE_ERROR') if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE careoncloud_webhook_subscription SET name = ?, endpoint_key = ?, event_patterns_json = ?, status = ?, version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ? AND version = ?',
        Bind => \@Bind,
    );
...
sub SubscriptionList {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => 'SELECT id FROM careoncloud_webhook_subscription WHERE tenant_id = ? ORDER BY id',
...
sub Scan {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('WEBHOOK_DISABLED') if !$Self->_Enabled();
    my $Limit = $Param{Limit} // $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Webhook::ScanBatchSize') // 100;
    return $Self->_Error('LIMIT_INVALID') if $Limit !~ m{\A[1-9][0-9]*\z}smx || $Limit > 500;
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare( SQL => "SELECT id, tenant_id FROM careoncloud_webhook_subscription WHERE status = 'active' ORDER BY tenant_id, id", Limit => 5000 );
    my @Subscriptions;
...
sub _ScanSubscription {
    ...
    $DB->Prepare(
        SQL => 'SELECT sequence_no, event_uuid, event_time, actor_type, actor_id, action_name, object_type, object_id, correlation_id, from_state, to_state, outcome, details_json, event_hash FROM careoncloud_audit_event WHERE tenant_id = ? AND sequence_no > ? ORDER BY sequence_no',
...
sub _ScanEvent {
    ...
    if ($Matched) {
        ...
        $Queue = $Kernel::OM->Get('Kernel::System::CareOnCloud::EscalationDispatcher')->QueueWebhook(
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Do(
        SQL => 'UPDATE careoncloud_webhook_subscription SET cursor_sequence = ?, change_time = current_timestamp WHERE tenant_id = ? AND id = ? AND cursor_sequence < ?',
...
sub _SubscriptionValidate {
    ...
    my $Configured = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Commitment::WebhookEndpoints');
...
sub _SubscriptionRowGet {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => "SELECT id, tenant_id, key_name, name, endpoint_key, event_patterns_json, status, cursor_sequence, version, create_time, create_by, change_time, change_by FROM careoncloud_webhook_subscription WHERE tenant_id = ? AND $Where = ?",
...
sub _AuditHead {
    ...
    $Kernel::OM->Get('Kernel::System::DB')->Prepare(
        SQL => 'SELECT last_sequence FROM careoncloud_audit_head WHERE tenant_id = ?', Bind => [ \$Param{TenantID} ], Limit => 1,
    );
...
sub _Audit {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->Record(
...
sub _Enabled { return $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Webhook::Enabled') ? 1 : 0 }
