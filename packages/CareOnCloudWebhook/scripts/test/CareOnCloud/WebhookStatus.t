...
my $Command = $Kernel::OM->Get('Kernel::System::Console::Command::Admin::CareOnCloud::WebhookStatus');
...
is( $Status->{Package}, 'CareOnCloudWebhook', 'status identifies package' );
is( $Status->{Version}, '0.3.0', 'status identifies version' );
ok( $Status->{Tables}->{careoncloud_webhook_subscription}, 'subscription table is present' );
