...
package Kernel::System::Console::Command::Admin::CareOnCloud::TenantGuardCheck;
...
sub Configure {
    ...
    $Self->AddOption(
        Name        => 'role',
        Description => 'Trusted server-side CareOnCloud role. Can be specified multiple times.',
        Required    => 1,
        HasValue    => 1,
        Multiple    => 1,
        ValueRegex  => qr/.+/smx,
    ...
    $Self->AddOption(
        Name        => 'action',
        Description => 'CareOnCloud action to authorize.',
        Required    => 1,
        HasValue    => 1,
        ValueRegex  => qr/.+/smx,
    );
