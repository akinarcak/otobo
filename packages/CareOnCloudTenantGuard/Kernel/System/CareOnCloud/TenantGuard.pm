...
package Kernel::System::CareOnCloud::TenantGuard;
...
sub DecisionGet {
    ...
    return $Deny->('DENY_POLICY_DISABLED') if !$ConfigObject->Get('CareOnCloud::TenantGuard::Enabled');
    ...
    if (
        ...
        && $ConfigObject->Get('CareOnCloud::TenantGuard::AllowPlatformAdmin')
        )
    {
        return {
            Allowed       => 1,
...
sub ScopeGet {
    ...
    return {
        ...
        PolicyVersion => $PolicyVersion,
        TenantIDs     => [],
    } if !$ConfigObject->Get('CareOnCloud::TenantGuard::Enabled');
...
sub _PolicyVersionGet {
    ...
    return $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TenantGuard::PolicyVersion') // 'unknown';
}
