...
# --
package Kernel::System::CareOnCloud::TenantCache;
...
sub Set {
    ...
    my $Maximum = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TenantCache::MaximumTTLSeconds') // 86400;
...
sub _Context {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('CACHE_DISABLED') if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TenantCache::Enabled');
    my $Valid = $Self->_Identifiers(%Param);
    return $Valid if !$Valid->{Success};
...
sub _Physical {
    ...
    return {
        Type => 'CareOnCloudT_' . substr( $TenantHash, 0, 24 ),
