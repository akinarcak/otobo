...
ok($Status->{Success},'audit schema is healthy'); is($Status->{Package},'D724Audit','status identifies package'); is($Status->{Version},'0.1.3','status identifies version'); is([sort keys %{$Status->{Tables}}],[qw(d724_audit_event d724_audit_head)],'both audit tables reported');
done_testing;
