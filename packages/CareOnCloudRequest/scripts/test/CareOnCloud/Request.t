...
for my $Role ( [ $TenantA, $AdminID, 'tenant_admin' ], [ $TenantB, $OtherID, 'agent' ] ) {
    ...
    ok(
        $DBObject->Do(
            SQL => 'INSERT INTO careoncloud_tenant_agent_role (tenant_id, user_id, role_name, status, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, ?, current_timestamp, ?, current_timestamp, ?)',
            Bind => \@Bind,
        ),
...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Request::Enabled', Value => 0 );
is(
