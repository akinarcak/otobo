...
package Kernel::Modules::AgentCareOnCloudRequest;
...
sub Run {
    ...
    if ( $Subaction eq 'ApprovalDecide' || $Subaction eq 'TaskUpdate' || $Subaction eq 'ResponseRecord' ) {
        ...
        return $Layout->Redirect( OP => "Action=AgentCareOnCloudRequest;TenantID=$TenantID" ) if $Result->{Success};
    ...
    my $Output = $Layout->Header( Title => 'CareOnCloud Requests' );
    $Output .= $Layout->NavigationBar();
    $Output .= $Layout->Notify( Priority => 'Error', Info => $Param{Error} ) if $Param{Error};
    $Output .= $Layout->Output( TemplateFile => 'AgentCareOnCloudRequest', Data => { TenantID => $TenantID } );
