...
package Kernel::System::CareOnCloud::Request;
...
our @ObjectDependencies = (
    'Kernel::Config',
    'Kernel::System::CareOnCloud::CatalogPortal',
...
sub _CustomerSubmit {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('REQUEST_DISABLED') if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Request::Enabled');
    ...
    my $Inserted = $DBObject->Do(
        SQL => 'INSERT INTO careoncloud_request '
    ...
    if ($ApprovalRequired) {
        ...
        return $Self->_InitializationFail( TenantID => $TenantID, RequestID => $RequestID, Actor => $Actor )
            if !$DBObject->Do(
                SQL => 'INSERT INTO careoncloud_request_approval (tenant_id, request_id, sequence_no, approver_role, status, decision_comment, version, create_time) '
    ...
    for my $Task ( @{ $Workflow->{fulfillment} } ) {
        ...
        return $Self->_InitializationFail( TenantID => $TenantID, RequestID => $RequestID, Actor => $Actor )
            if !$DBObject->Do(
                SQL => 'INSERT INTO careoncloud_request_task (tenant_id, request_id, key_name, name, task_type, status, version, result_comment, create_time, change_time, change_by) '
    ...
    return $Self->_InitializationFail( TenantID => $TenantID, RequestID => $RequestID, Actor => $Actor )
        if !$DBObject->Do(
            SQL => 'UPDATE careoncloud_request SET request_number = ?, status = ?, change_time = current_timestamp, change_by = ? '
...
sub CustomerGet {
    ...
    my $Context = $Kernel::OM->Get('Kernel::System::CareOnCloud::CatalogPortal')->ContextGet(
        CustomerUserID => $Param{CustomerUserID}, CustomerID => $Param{CustomerID},
    );
    return $Context if !$Context->{Success};
...
sub AgentList {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => "SELECT id FROM careoncloud_request WHERE tenant_id = ? AND status <> 'initializing' AND status <> 'submission_failed' ORDER BY id DESC",
...
sub _ApprovalDecide {
    ...
    return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
        SQL => 'UPDATE careoncloud_request_approval SET status = ?, decision_comment = ?, decision_time = current_timestamp, decision_by = ?, version = version + 1 '
    ...
    return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
        SQL => "UPDATE careoncloud_request SET status = ?, version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ? AND status = 'awaiting_approval'",
        Bind => \@RequestBind,
    );
    my $Commitment = $Self->_CommitmentObject();
    if ($Commitment) {
    ...
    my @TaskBind = map { \$_ } @TaskValues;
    $DBObject->Do(
        SQL => "UPDATE careoncloud_request_task SET status = ?, version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND request_id = ? AND status = 'blocked'",
        Bind => \@TaskBind,
    );
...
sub _TaskUpdate {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
        SQL => 'UPDATE careoncloud_request_task SET status = ?, result_comment = ?, version = version + 1, change_time = current_timestamp, change_by = ? '
            . 'WHERE tenant_id = ? AND id = ? AND version = ?', Bind => \@Bind,
    );
    ...
    if ( $Param{Status} eq 'failed' ) {
        ...
        my @RequestBind = map { \$_ } @RequestValues;
        return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
            SQL => "UPDATE careoncloud_request SET status = 'fulfillment_failed', version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ? AND status = 'in_fulfillment'",
            Bind => \@RequestBind,
        );
    ...
    if ( $Param{Status} eq 'completed' ) {
        ...
        $DBObject->Prepare(
            SQL => "SELECT COUNT(*) FROM careoncloud_request_task WHERE tenant_id = ? AND request_id = ? AND status <> 'completed'",
        ...
        if (!$Remaining) {
            ...
            my @RequestBind = map { \$_ } @RequestValues;
            return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
                SQL => "UPDATE careoncloud_request SET status = 'fulfilled', version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ?",
                Bind => \@RequestBind,
            );
            my $Commitment = $Self->_CommitmentObject();
            if ($Commitment) {
...
sub _CommitmentObject {
    my ($Self) = @_;
    return if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Commitment::Enabled');
    my $Object = eval { $Kernel::OM->Get('Kernel::System::CareOnCloud::Commitment') };
    return $@ ? undef : $Object;
}

sub _AuditRecord {
    my ( $Self, %Param ) = @_;
    my $Audit = eval { $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit') };
...
sub _TransactionRun {
    ...
    if ( !$OK ) {
        ...
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            Priority => 'error', Message => "CareOnCloud request transaction failed: $Failure",
        );
...
sub _RequestByIdempotency {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id, payload_hash FROM careoncloud_request WHERE tenant_id = ? AND requester_id = ? AND idempotency_key = ?',
...
sub _RequestAggregate {
    ...
    $DBObject->Prepare(
        SQL => 'SELECT id, request_number, tenant_id, catalog_item_id, requester_id, status, version, answers_json, workflow_json, payload_hash, create_time, create_by, change_time, change_by '
            . 'FROM careoncloud_request WHERE tenant_id = ? AND id = ?', Bind => [ \$Param{TenantID}, \$Param{RequestID} ], Limit => 1,
    );
...
sub _ApprovalsGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id, sequence_no, approver_role, status, decision_comment, version, decision_time, decision_by FROM careoncloud_request_approval WHERE tenant_id = ? AND request_id = ? ORDER BY sequence_no',
...
sub _TasksGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id, key_name, name, task_type, status, version, result_comment, assigned_group, change_time, change_by FROM careoncloud_request_task WHERE tenant_id = ? AND request_id = ? ORDER BY id',
        Bind => [ \$Param{TenantID}, \$Param{RequestID} ],
    );
    my @Data;
    while ( my @Row = $DBObject->FetchrowArray() ) {
...
sub _TaskRowGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT request_id, status, version FROM careoncloud_request_task WHERE tenant_id = ? AND id = ?',
...
sub _ApprovalRowGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT status, version FROM careoncloud_request_approval WHERE tenant_id = ? AND id = ?',
...
sub _InitializationFail {
    ...
    $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => "UPDATE careoncloud_request SET status = 'submission_failed', change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ?",
        Bind => \@Bind,
    );
