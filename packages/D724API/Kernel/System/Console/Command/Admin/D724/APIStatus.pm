...
our $VERSION = '0.1.1'; our @ObjectDependencies = ('Kernel::Config','Kernel::System::DB','Kernel::System::JSON');
...
sub StatusData {
    ...
    my %Count=(Clients=>0,ActiveClients=>0,Tokens=>0,ActiveTokens=>0,InvalidTenantClients=>0,InvalidSecretHashes=>0,InvalidTokenHashes=>0,DuplicateRateWindows=>0,RateWindowUnique=>0);
    if(!@Missing){
        ...
        $DB->Prepare(SQL=>"SELECT COUNT(*) FROM d724_api_token WHERE token_hash NOT REGEXP '^[0-9a-f]{64}$'"); ($Count{InvalidTokenHashes})=$DB->FetchrowArray();
    ...
    my $Success=!@Missing && $Count{RateWindowUnique} && !$Count{InvalidTenantClients} && !$Count{InvalidSecretHashes} && !$Count{InvalidTokenHashes} && !$Count{DuplicateRateWindows};
    return {Success=>$Success?1:0,Package=>'D724API',Version=>'0.1.1',Enabled=>$Kernel::OM->Get('Kernel::Config')->Get('D724::API::Enabled')?1:0,MissingTables=>\@Missing,Counts=>\%Count};
}
sub Run { my ($Self)=@_; my $S=$Self->StatusData(); if($Self->GetOption('json')){$Self->Print($Kernel::OM->Get('Kernel::System::JSON')->Encode(Data=>$S,SortKeys=>1,Pretty=>1)."\n")}else{$Self->Print('D724API 0.1.1: '.($S->{Success}?'OK':'FAILED')."\n")} return $S->{Success}?$Self->ExitCodeOk():$Self->ExitCodeError() }
1;
