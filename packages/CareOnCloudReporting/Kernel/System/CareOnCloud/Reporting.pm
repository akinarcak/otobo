...
# --
package Kernel::System::CareOnCloud::Reporting;
...
our @ObjectDependencies = (
    'Kernel::Config', 'Kernel::System::DB', 'Kernel::System::CareOnCloud::TenantDirectory',
    'Kernel::System::CareOnCloud::TenantCache', 'Kernel::System::CareOnCloud::TenantGuard', 'Kernel::System::JSON',
);
...
sub CustomReport {
    ...
    my $SQL = 'SELECT ' . join( ', ', @Select )
        . ' FROM careoncloud_request r'
        . ' INNER JOIN careoncloud_catalog_item c ON c.tenant_id = r.tenant_id AND c.id = r.catalog_item_id'
        . ' INNER JOIN careoncloud_service_offering o ON o.tenant_id = c.tenant_id AND o.id = c.offering_id'
        . ' INNER JOIN careoncloud_service s ON s.tenant_id = o.tenant_id AND s.id = o.service_id'
        . ' LEFT JOIN careoncloud_commitment_instance i ON i.tenant_id = r.tenant_id AND i.request_id = r.id'
...
sub DefinitionCreate {
    ...
    return $Self->_Error('CREATE_FAILED') if !$DB->Do(
        SQL => 'INSERT INTO careoncloud_report_definition (tenant_id, key_name, name, description, owner_user_id, visibility, definition_json, version, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, ?, ?, ?, ?, 1, current_timestamp, ?, current_timestamp, ?)',
...
sub DefinitionList {
    ...
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => 'SELECT id, key_name, name, description, owner_user_id, visibility, definition_json, version, create_time, change_time FROM careoncloud_report_definition WHERE tenant_id = ? AND (owner_user_id = ? OR visibility = ?) ORDER BY name, id',
...
sub DefinitionGet {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => "SELECT id, key_name, name, description, owner_user_id, visibility, definition_json, version, create_time, change_time FROM careoncloud_report_definition WHERE tenant_id = ? AND $Where AND (owner_user_id = ? OR visibility = ?)",
...
sub DefinitionDelete {
    ...
    return $Self->_Error('DELETE_FAILED') if !$DB->Do(
        SQL => 'DELETE FROM careoncloud_report_definition WHERE tenant_id = ? AND id = ? AND owner_user_id = ?',
...
sub Export {
    ...
    my $TenantFile = $Param{TenantID};
    $TenantFile =~ s{[^a-zA-Z0-9_-]}{_}gsmx;
    my $Name = "careoncloud-operational-$TenantFile-$Param{From}-$Param{To}.$Param{Format}";
    if ( $Param{Format} eq 'json' ) {
        return {
...
sub TenantLabelGet {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('REPORTING_DISABLED') if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Reporting::Enabled');
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => 'SELECT name FROM careoncloud_tenant WHERE key_name = ? AND status = ?', Bind => [ \$TenantID, \$Status ], Limit => 1,
    );
...
sub _Validate {
    ...
    my $Maximum = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Reporting::MaximumRangeDays') // 366;
...
sub _SummaryQuery {
    ...
    my @RequestStatus;
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => 'SELECT status, COUNT(*) FROM careoncloud_request WHERE tenant_id = ? AND create_time >= ? AND create_time < DATE_ADD(?, INTERVAL 1 DAY) GROUP BY status ORDER BY status',
    ...
    my @CatalogItems;
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => 'SELECT c.key_name, c.name, COUNT(r.id) FROM careoncloud_catalog_item c INNER JOIN careoncloud_request r ON r.tenant_id = c.tenant_id AND r.catalog_item_id = c.id WHERE c.tenant_id = ? AND r.create_time >= ? AND r.create_time < DATE_ADD(?, INTERVAL 1 DAY) GROUP BY c.id, c.key_name, c.name ORDER BY c.key_name',
        Bind => [ \$Param{TenantID}, \$Param{From}, \$Param{To} ],
    );
    ...
    my @Commitments;
    return $Self->_Error('QUERY_FAILED') if !$DB->Prepare(
        SQL => 'SELECT i.objective_type, i.status, COUNT(*) FROM careoncloud_commitment_instance i INNER JOIN careoncloud_request r ON r.tenant_id = i.tenant_id AND r.id = i.request_id WHERE i.tenant_id = ? AND r.create_time >= ? AND r.create_time < DATE_ADD(?, INTERVAL 1 DAY) GROUP BY i.objective_type, i.status ORDER BY i.objective_type, i.status',
        Bind => [ \$Param{TenantID}, \$Param{From}, \$Param{To} ],
    );
...
sub _SummaryCached {
    my ( $Self, %Param ) = @_;
    my $TTL = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Reporting::CacheTTLSeconds') // 60;
