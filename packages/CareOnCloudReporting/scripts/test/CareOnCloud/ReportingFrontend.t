...
my$H=$L->Output(TemplateFile=>'AgentCareOnCloudOperations',Data=>{TenantID=>'demo-bank',From=>'2026-07-01',To=>'2026-07-31',Status=>q{},Requests=>7,Commitments=>10,BreachedCommitments=>1,Compliance=>90,SavedNotice=>1,CustomColumns=>['Status','Requests'],CustomRows=>[{Values=>['fulfilled <x>',7]}]});
like($H,qr{CareOnCloud Operations Center},'operations title rendered');like($H,qr{\[DEMO\]\s*Bank\s*&lt;A&gt;},'tenant label escaped');like($H,qr{Cloud\s*&lt;script&gt;x&lt;/script&gt;},'catalog label escaped');unlike($H,qr{<script>x</script>},'untrusted markup never rendered');like($H,qr{90%},'SLA compliance KPI rendered');like($H,qr{name="TenantID"},'tenant scope is submitted explicitly');like($H,qr{id="CareOnCloudReportDimensions"},'custom dimensions selector rendered');like($H,qr{id="CareOnCloudReportMetrics"},'custom metrics selector rendered');like($H,qr{fulfilled\s*&lt;x&gt;},'custom report values escaped');like($H,qr{id="CareOnCloudSavedReport"},'saved report selector rendered');like($H,qr{Executive\s*&lt;unsafe&gt;},'saved report name escaped');like($H,qr{name="ChallengeToken"},'saved report mutation carries CSRF token');done_testing;
