...
# --
package Kernel::Modules::AgentD724Change;
...
sub Run{
 ...
 my$D=$Kernel::OM->Get('Kernel::System::D724::TenantDirectory');my$G=$Kernel::OM->Get('Kernel::System::D724::TenantGuard');my$C=$Kernel::OM->Get('Kernel::System::D724::Change');
 ...
 my$Action=$S->{Subaction}//q{};if($Action){$L->ChallengeTokenCheck();my%X=(Subject=>$Sub,TenantID=>$Tenant,UserID=>$S->{UserID});my$R;
  ...
  return$L->Redirect(OP=>"Action=AgentD724Change;TenantID=$Tenant")if$R->{Success};$P{Error}=$R->{Error};
 }
 ...
 $O.=$L->Output(TemplateFile=>'AgentD724Change',Data=>{TenantID=>$Tenant,Total=>scalar@{$List->{Data}},Awaiting=>$Count{awaiting_approval}//0,Scheduled=>$Count{scheduled}//0,Implementing=>$Count{implementing}//0,CanManage=>$CanManage});$O.=$L->Footer();return$O;
}
1;
