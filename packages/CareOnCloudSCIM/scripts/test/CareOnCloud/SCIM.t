...
my $API = $Kernel::OM->Get('Kernel::System::CareOnCloud::APIAuth');
my $SCIM = $Kernel::OM->Get('Kernel::System::CareOnCloud::SCIM');
my $Suffix = lc $Helper->GetRandomID();
my $TenantA = "scim-a-$Suffix";
my $TenantB = "scim-b-$Suffix";

for my $Setting (
    [ 'CareOnCloud::SCIM::Enabled', 1 ], [ 'CareOnCloud::API::Enabled', 1 ], [ 'CareOnCloud::API::BcryptCost', 9 ],
    [ 'CareOnCloud::API::TokenTTLMax', 900 ], [ 'CareOnCloud::API::RateLimitMax', 200 ],
...
for my $T ($TenantA,$TenantB) {
    ...
    ok($DB->Do(SQL=>"INSERT INTO careoncloud_tenant (key_name,name,status,version,create_time,create_by,change_time,change_by) VALUES (?,?,'active',1,current_timestamp,?,current_timestamp,?)",Bind=>\@B),"tenant $T created");
}
...
$DB->Prepare(SQL=>"SELECT status FROM careoncloud_tenant_agent_role r INNER JOIN careoncloud_scim_user u ON u.native_user_id=r.user_id WHERE u.scim_id=? AND r.tenant_id=? AND r.role_name='agent'",Bind=>[\$AgentID,\$TenantA],Limit=>1);my($AgentRole)=$DB->FetchrowArray();is($AgentRole,'active','group membership grants tenant agent role');
...
$DB->Prepare(SQL=>"SELECT COUNT(*) FROM careoncloud_tenant_agent_role r INNER JOIN careoncloud_scim_user u ON u.native_user_id=r.user_id WHERE u.scim_id=? AND r.tenant_id=? AND r.status='active'",Bind=>[\$AgentID,\$TenantA]);my($ActiveRoles)=$DB->FetchrowArray();is($ActiveRoles,0,'deprovision revokes every role in the tenant');
...
$DB->Prepare(SQL=>"SELECT COUNT(*) FROM careoncloud_tenant_agent_role r INNER JOIN careoncloud_scim_user u ON u.native_user_id=r.user_id WHERE u.scim_id=? AND r.tenant_id=? AND r.status='active'",Bind=>[\$AgentID,\$TenantA]);($ActiveRoles)=$DB->FetchrowArray();is($ActiveRoles,2,'reactivation restores baseline and group-derived roles');
...
my $Audit=$Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->List(Subject=>$Admin,TenantID=>$TenantA,Limit=>100);
...
$DB->Prepare(SQL=>'SELECT native_user_id FROM careoncloud_scim_user WHERE tenant_id=? AND scim_id=?',Bind=>[\$TenantA,\$AgentID],Limit=>1);my($NativeAgentID)=$DB->FetchrowArray();
my $CustomerLogin="scim.customer.$Suffix";my $ClientID=$Client->{Data}->{ClientID};
for my $Delete (
    [ 'DELETE FROM careoncloud_scim_group_member WHERE tenant_id=?', [\$TenantA] ],
    [ 'DELETE FROM careoncloud_scim_group WHERE tenant_id=?', [\$TenantA] ],
    [ 'DELETE FROM careoncloud_tenant_agent_role WHERE tenant_id=?', [\$TenantA] ],
    [ 'DELETE FROM careoncloud_scim_user WHERE tenant_id=?', [\$TenantA] ],
    [ 'DELETE FROM careoncloud_api_rate WHERE client_id=?', [\$ClientID] ],
    [ 'DELETE FROM careoncloud_api_token WHERE client_id=?', [\$ClientID] ],
    [ 'DELETE FROM careoncloud_api_client WHERE client_id=?', [\$ClientID] ],
    [ 'DELETE FROM careoncloud_audit_event WHERE tenant_id=?', [\$TenantA] ],
    [ 'DELETE FROM careoncloud_audit_head WHERE tenant_id=?', [\$TenantA] ],
...
for my $T($TenantA,$TenantB){$DB->Do(SQL=>'DELETE FROM careoncloud_audit_event WHERE tenant_id=?',Bind=>[\$T]);$DB->Do(SQL=>'DELETE FROM careoncloud_audit_head WHERE tenant_id=?',Bind=>[\$T]);ok($DB->Do(SQL=>'DELETE FROM careoncloud_tenant WHERE key_name=?',Bind=>[\$T]),"tenant fixture $T removed")}
