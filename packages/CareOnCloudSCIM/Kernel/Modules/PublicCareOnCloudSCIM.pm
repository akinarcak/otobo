...
# --
package Kernel::Modules::PublicCareOnCloudSCIM;
...
sub _Run {
    ...
    if ( $Route =~ m{\A(?:service_provider_config|schemas|resource_types)\z}smx ) {
        ...
        my $Auth = $Kernel::OM->Get('Kernel::System::CareOnCloud::APIAuth')->Authorize( AccessToken => $Bearer, TenantID => $TenantID, Action => 'scim.provision' );
...
sub _UserFields {
    ...
    return (UserName=>$P->{userName},ExternalID=>$P->{externalId},GivenName=>ref$P->{name}eq'HASH'?$P->{name}->{givenName}:undef,FamilyName=>ref$P->{name}eq'HASH'?$P->{name}->{familyName}:undef,Email=>$Email,Active=>exists$P->{active}?$P->{active}:1,Surface=>$E->{surface}//$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::SCIM::DefaultSurface')//'agent');
}
