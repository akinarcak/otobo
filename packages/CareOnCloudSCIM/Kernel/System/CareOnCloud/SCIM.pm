...
# --
package Kernel::System::CareOnCloud::SCIM;
...
our @ObjectDependencies = (
    'Kernel::Config', 'Kernel::System::CareOnCloud::APIAuth', 'Kernel::System::CareOnCloud::Audit',
...
sub UserCreate {
    ...
    my $Result = $Self->_TransactionRun( Code => sub {
        ...
        die "SCIM_USER_INSERT_FAILED\n" if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'INSERT INTO careoncloud_scim_user (scim_id, tenant_id, external_id, surface, user_name, email, given_name, family_name, active, native_user_id, customer_login, version, create_time, change_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, current_timestamp, current_timestamp)',
            Bind => \@Bind,
        );
...
sub UserList {
    ...
    my $Max = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::SCIM::ListLimitMax') // 200;
    ...
    $DB->Prepare( SQL => "SELECT COUNT(*) FROM careoncloud_scim_user WHERE $Where", Bind => \@Bind ); my ($Total) = $DB->FetchrowArray();
    $DB->Prepare(
        SQL => "SELECT scim_id, tenant_id, external_id, surface, user_name, email, given_name, family_name, active, native_user_id, customer_login, version, create_time, change_time FROM careoncloud_scim_user WHERE $Where ORDER BY scim_id",
...
sub UserReplace {
    ...
    my $Result = $Self->_TransactionRun( Code => sub {
        ...
        die "VERSION_CONFLICT\n" if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'UPDATE careoncloud_scim_user SET email = ?, given_name = ?, family_name = ?, active = ?, version = version + 1, change_time = current_timestamp WHERE tenant_id = ? AND scim_id = ? AND version = ?', Bind => \@Bind,
        );
...
sub GroupCreate {
    ...
    my $Result = $Self->_TransactionRun( Code => sub {
        ...
        die "SCIM_GROUP_INSERT_FAILED\n" if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'INSERT INTO careoncloud_scim_group (scim_id, tenant_id, external_id, display_name, role_name, version, create_time, change_time) VALUES (?, ?, ?, ?, ?, 1, current_timestamp, current_timestamp)', Bind => \@Bind,
        );
...
sub GroupList {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB'); my @Bind = map { \$_ } @Values;
    $DB->Prepare( SQL => "SELECT COUNT(*) FROM careoncloud_scim_group WHERE $Where", Bind => \@Bind ); my ($Total) = $DB->FetchrowArray();
    $DB->Prepare( SQL => "SELECT scim_id,tenant_id,external_id,display_name,role_name,version,create_time,change_time FROM careoncloud_scim_group WHERE $Where ORDER BY scim_id", Bind => \@Bind, Limit => $Count, Offset => $Start - 1 );
...
sub GroupReplace {
    ...
    my $Result = $Self->_TransactionRun( Code => sub {
        ...
        die "VERSION_CONFLICT\n" if !$Kernel::OM->Get('Kernel::System::DB')->Do( SQL => 'UPDATE careoncloud_scim_group SET display_name = ?, role_name = ?, version = version + 1, change_time = current_timestamp WHERE tenant_id = ? AND scim_id = ? AND version = ?', Bind => \@Bind );
...
sub GroupDelete {
    ...
    my $Result = $Self->_TransactionRun( Code => sub {
        ...
        $DB->Prepare(SQL=>'SELECT user_id FROM careoncloud_scim_group_member WHERE tenant_id = ? AND group_id = ?',Bind=>[\$T,\$G]);my @IDs;while(my($ID)=$DB->FetchrowArray()){push @IDs,$ID}
        $DB->Do(SQL=>'DELETE FROM careoncloud_scim_group_member WHERE tenant_id = ? AND group_id = ?',Bind=>[\$T,\$G]) || die "MEMBER_DELETE_FAILED\n";
        $DB->Do(SQL=>'DELETE FROM careoncloud_scim_group WHERE tenant_id = ? AND scim_id = ? AND version = ?',Bind=>[\$T,\$G,\$Current->{Version}]) || die "VERSION_CONFLICT\n";
...
sub _Authorize {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('SCIM_DISABLED') if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::SCIM::Enabled');
    return $Self->_Error('TENANT_ID_INVALID') if ( $Param{TenantID} // q{} ) !~ m{\A[a-z0-9][a-z0-9_-]{1,127}\z}smx;
    return $Kernel::OM->Get('Kernel::System::CareOnCloud::APIAuth')->Authorize( AccessToken => $Param{AccessToken}, TenantID => $Param{TenantID}, Action => 'scim.provision' );
}
...
sub _GroupMembersReplace {
    ...
    $DB->Prepare( SQL => 'SELECT user_id FROM careoncloud_scim_group_member WHERE tenant_id = ? AND group_id = ?', Bind => [ \$Tenant, \$Group ] );
    ...
    for my $ID ( keys %Wanted ) {
        ...
        if ( !$Current{$ID} ) { $DB->Do( SQL => 'INSERT INTO careoncloud_scim_group_member (group_id, user_id, tenant_id, create_time) VALUES (?, ?, ?, current_timestamp)', Bind => [ \$Group, \$ID, \$Tenant ] ) || die "MEMBER_INSERT_FAILED\n" }
    ...
    for my $ID ( keys %Current ) {
        ...
        $DB->Do( SQL => 'DELETE FROM careoncloud_scim_group_member WHERE tenant_id = ? AND group_id = ? AND user_id = ?', Bind => [ \$Tenant, \$Group, \$ID ] ) || die "MEMBER_DELETE_FAILED\n";
...
sub _RoleReconcile {
    ...
    $DB->Prepare( SQL => 'SELECT 1 FROM careoncloud_scim_group_member m INNER JOIN careoncloud_scim_group g ON g.scim_id = m.group_id AND g.tenant_id = m.tenant_id WHERE m.tenant_id = ? AND m.user_id = ? AND g.role_name = ?', Bind => [ \$Tenant, \$User->{SCIMID}, \$Role ], Limit => 1 ); my ($Keep)=$DB->FetchrowArray();
...
sub _MembershipSet {
    ...
    $DB->Prepare( SQL => 'SELECT id, status FROM careoncloud_tenant_agent_role WHERE tenant_id = ? AND user_id = ? AND role_name = ? FOR UPDATE', Bind => [ \$T, \$U, \$R ] ); my ($ID,$Status)=$DB->FetchrowArray(); my $Want=$Param{Active}?'active':'revoked';
    return 1 if $ID && $Status eq $Want;
    if ($ID) { $DB->Do( SQL => 'UPDATE careoncloud_tenant_agent_role SET status = ?, version = version + 1, change_time = current_timestamp, change_by = 1 WHERE id = ?', Bind => [ \$Want, \$ID ] ) || die "MEMBERSHIP_UPDATE_FAILED\n" }
    elsif ($Param{Active}) { $DB->Do( SQL => "INSERT INTO careoncloud_tenant_agent_role (tenant_id,user_id,role_name,status,version,create_time,create_by,change_time,change_by) VALUES (?,?,?,'active',1,current_timestamp,1,current_timestamp,1)", Bind => [ \$T, \$U, \$R ] ) || die "MEMBERSHIP_INSERT_FAILED\n" }
    return 1;
}
...
sub _UserMembershipsReconcile {
    ...
    if (!$Param{Active}) {
        $DB->Do( SQL => "UPDATE careoncloud_tenant_agent_role SET status = 'revoked', version = version + 1, change_time = current_timestamp, change_by = 1 WHERE tenant_id = ? AND user_id = ? AND status = 'active'", Bind => [ \$T, \$U->{NativeUserID} ] ) || die "MEMBERSHIP_UPDATE_FAILED\n";
        return 1;
    }
    $Self->_MembershipSet(TenantID=>$T,NativeUserID=>$U->{NativeUserID},Role=>'requester',Active=>1);
    $DB->Prepare(SQL=>'SELECT DISTINCT g.role_name FROM careoncloud_scim_group_member m INNER JOIN careoncloud_scim_group g ON g.scim_id=m.group_id AND g.tenant_id=m.tenant_id WHERE m.tenant_id=? AND m.user_id=?',Bind=>[\$T,\$U->{SCIMID}]);
...
sub _AgentHasActiveMembership {
    ...
    $DB->Prepare( SQL => "SELECT 1 FROM careoncloud_tenant_agent_role WHERE user_id = ? AND status = 'active'", Bind => [ \$U ], Limit => 1 ); my ($Has)=$DB->FetchrowArray(); return 1 if $Has;
    return 0;
}
...
sub _Audit { my($Self,%P)=@_;my $A=$P{Auth}->{Data};my $R=$Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->Record(TenantID=>$P{TenantID},ActorType=>'integration',ActorID=>'integration:'.$A->{ClientID},Action=>$P{Action},ObjectType=>$P{ObjectType},ObjectID=>$P{ObjectID},CorrelationID=>'scim:'.$P{ObjectID},DedupeKey=>'scim:'.$P{ObjectID}.':version:'.$P{Version},FromState=>$P{From},ToState=>$P{To},Outcome=>'success',Details=>$P{Details});die "AUDIT_WRITE_FAILED\n" if !$R->{Success};return 1 }
