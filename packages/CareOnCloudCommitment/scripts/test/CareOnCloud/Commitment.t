...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Request::Enabled', Value => 1 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::Enabled', Value => 1 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::EscalationDispatchEnabled', Value => 1 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::EscalationBatchSize', Value => 25 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::EscalationMaxAttempts', Value => 3 );
...
for my $Tenant ( $TenantA, $TenantB ) {
    ...
    $DBObject->Do(
        SQL => 'INSERT INTO careoncloud_tenant (key_name, name, status, version, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, ?, current_timestamp, ?, current_timestamp, ?)', Bind => \@Bind,
...
for my $Role ( [ $TenantA, $AdminID, 'tenant_admin' ], [ $TenantA, $AdminID, 'service_owner' ], [ $TenantB, $OtherID, 'agent' ] ) {
    ...
    $DBObject->Do(
        SQL => 'INSERT INTO careoncloud_tenant_agent_role (tenant_id, user_id, role_name, status, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, ?, current_timestamp, ?, current_timestamp, ?)', Bind => \@Bind,
...
my $RequestObject = $Kernel::OM->Get('Kernel::System::CareOnCloud::Request');
my $NewRequest = sub {
    my ($Key) = @_;
...
$DBObject->Do(
    SQL => 'UPDATE careoncloud_escalation_outbox SET status = ?, attempt_count = ?, available_time = ?, processed_time = NULL, delivery_ref = ?, response_code = ? WHERE id = ?',
...
is( $EmailCall{CustomHeaders}->{'X-CareOnCloud-Tenant'}, $TenantA, 'notification carries tenant audit header' );
...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::WebhookAllowedHosts', Value => ['hooks.example.test'] );
$Helper->ConfigSettingChange(
    Key => 'CareOnCloud::Commitment::WebhookEndpoints',
    Value => { 'audit-hook::URL' => 'https://hooks.example.test/careoncloud', 'audit-hook::Secret' => '0123456789abcdef0123456789abcdef' },
);
my %WebhookCall;
{
    no warnings 'redefine';
...
like( $WebhookCall{Header}->{'X-CareOnCloud-Signature-256'}, qr{\Asha256=[0-9a-f]{64}\z}, 'webhook carries an HMAC SHA-256 signature' );
is( $WebhookCall{Header}->{'X-CareOnCloud-Signature-Version'}, 'v1', 'webhook declares its signature contract version' );
is( $WebhookCall{Header}->{'X-CareOnCloud-Signature-Timestamp'}, '2026-07-27 12:34:56', 'webhook signature includes a replay-window timestamp' );
is( $WebhookCall{Header}->{'X-CareOnCloud-Delivery-ID'}, 9002, 'webhook carries stable delivery identifier' );
is( $WebhookCall{Header}->{'X-CareOnCloud-Tenant'}, $TenantA, 'webhook carries the tenant boundary as signed-delivery context' );
is(
    $WebhookCall{Header}->{'X-CareOnCloud-Signature-256'},
...
ok(
    $DBObject->Do(
        SQL => 'UPDATE careoncloud_commitment_instance SET status = ? WHERE tenant_id = ? AND id = ?',
        Bind => [ \$Running, \$TenantA, \$GuardedCommitmentID ],
    ),
    'daemon guard commitment fixture is made runnable',
);
ok(
    $DBObject->Do( SQL => 'UPDATE careoncloud_tenant SET status = ? WHERE key_name = ?', Bind => [ \$InactiveTenant, \$TenantA ] ),
...
$DBObject->Prepare(
    SQL => 'SELECT status, version FROM careoncloud_commitment_instance WHERE tenant_id = ? AND id = ?',
...
$DBObject->Prepare( SQL => 'SELECT status, lease_token FROM careoncloud_escalation_outbox WHERE id = ?', Bind => [ \$EscalationID ], Limit => 1 );
is( [ $DBObject->FetchrowArray() ], [ 'pending', q{} ], 'denied delivery remains unclaimed and unchanged' );

$Helper->ConfigSettingChange( Key => 'CareOnCloud::Commitment::Enabled', Value => 0 );
is(
