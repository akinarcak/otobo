...
package Kernel::Modules::AdminCareOnCloudCommitment;
...
sub Run {
    ...
    if ( ( $Self->{Subaction} // q{} ) eq 'PolicySave' ) {
        ...
        return $Layout->Redirect( OP => "Action=AdminCareOnCloudCommitment;TenantID=$TenantID" ) if $Result->{Success};
    ...
    my $Output = $Layout->Header( Title => 'CareOnCloud Commitments' );
    $Output .= $Layout->NavigationBar();
    $Output .= $Layout->Notify( Priority => 'Error', Info => $Param{Error} ) if $Param{Error};
    $Output .= $Layout->Output( TemplateFile => 'AdminCareOnCloudCommitment', Data => { TenantID => $TenantID } );
