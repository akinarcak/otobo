...
# --
package Kernel::System::Console::Command::Admin::CareOnCloud::CommitmentReplay;
...
sub Run {
    ...
    my $Context = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory')->ContextGet( UserID => $UserID );
    if ( !$Context->{Success} ) {
        $Self->PrintError("Actor context failed: $Context->{Error}");
        return $Self->ExitCodeError();
    }
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::EscalationDispatcher')->Replay(
