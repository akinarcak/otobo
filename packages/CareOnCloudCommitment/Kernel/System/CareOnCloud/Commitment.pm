...
package Kernel::System::CareOnCloud::Commitment;
...
sub PolicyCreate {
    ...
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'INSERT INTO careoncloud_commitment_policy (tenant_id, key_name, name, calendar_id, target_seconds, warning_percent, pause_statuses_json, status, version, create_time, create_by, change_time, change_by) '
...
sub PolicyUpdate {
    ...
    my @Bind = map { \$_ } @Values;
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE careoncloud_commitment_policy SET name = ?, calendar_id = ?, target_seconds = ?, warning_percent = ?, pause_statuses_json = ?, status = ?, '
...
sub PolicyList {
    ...
    $DBObject->Prepare( SQL => 'SELECT id FROM careoncloud_commitment_policy WHERE tenant_id = ? ORDER BY key_name', Bind => [ \$TenantID ], Limit => 500 );
    my @Data;
...
sub Start {
    ...
    my @Bind = map { \$_ } @Values;
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'INSERT INTO careoncloud_commitment_instance (tenant_id, request_id, policy_id, status, objective_key, objective_type, start_signal, stop_signal, escalation_actions_json, '
...
sub CustomerGetByRequest {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id FROM careoncloud_request WHERE tenant_id = ? AND id = ? AND requester_id = ?',
...
sub Sweep {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare( SQL => "SELECT id, tenant_id, version FROM careoncloud_commitment_instance WHERE status IN ('running', 'warning') ORDER BY id", Limit => 5000 );
    my @Work;
...
sub _EvaluateOne {
    ...
    my @Bind = map { \$_ } @Values;
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE careoncloud_commitment_instance SET status = ?, breached_at = ?, version = version + 1, change_time = current_timestamp, change_by = ? '
...
sub _Transition {
    ...
    my @Bind = map { \$_ } @Values;
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE careoncloud_commitment_instance SET status = ?, consumed_seconds = ?, running_since = ?, paused_at = ?, warning_time = ?, due_time = ?, breached_at = ?, met_at = ?, '
            . 'version = version + 1, change_time = current_timestamp, change_by = ? WHERE tenant_id = ? AND id = ? AND version = ?', Bind => \@Bind,
    );
    return $Self->_Error('DATABASE_ERROR') if !$OK;
    my $Updated = $Self->_InstanceRowGet( TenantID => $Param{TenantID}, CommitmentID => $Param{CommitmentID} );
...
sub _AgentAuthorize {
    ...
    if ( defined $Param{IntegrationSubject} ) {
        ...
        $Context = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory')->ContextGet( UserID => $Param{UserID} );
...
sub _PolicyObjectivesReplace {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Do(
        SQL => 'DELETE FROM careoncloud_commitment_objective WHERE tenant_id = ? AND policy_id = ?',
    ...
    for my $Objective ( @{ $Param{Objectives} } ) {
        ...
        return $Self->_Error('DATABASE_ERROR') if !$DBObject->Do(
            SQL => 'INSERT INTO careoncloud_commitment_objective (tenant_id, policy_id, key_name, objective_type, target_seconds, warning_percent, start_signal, stop_signal, escalation_actions_json, sequence_no) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            Bind => \@Bind,
        );
    }
    return { Success => 1 };
...
sub _PolicyObjectivesGet {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT key_name, objective_type, target_seconds, warning_percent, start_signal, stop_signal, escalation_actions_json FROM careoncloud_commitment_objective WHERE tenant_id = ? AND policy_id = ? ORDER BY sequence_no, id',
        Bind => [ \$Param{TenantID}, \$Param{PolicyID} ],
    );
    my @Data;
...
sub _PolicyRowGet {
    ...
    $DBObject->Prepare(
        SQL => "SELECT id, tenant_id, key_name, name, calendar_id, target_seconds, warning_percent, pause_statuses_json, status, version, create_time, create_by, change_time, change_by FROM careoncloud_commitment_policy WHERE tenant_id = ? AND $Where = ?",
...
sub _InstancesByRequest {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id FROM careoncloud_commitment_instance WHERE tenant_id = ? AND request_id = ? ORDER BY id',
...
sub _Aggregate {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    $DBObject->Prepare(
        SQL => 'SELECT id, event_type, event_time, from_status, to_status, consumed_seconds, actor, reason FROM careoncloud_commitment_event WHERE tenant_id = ? AND commitment_id = ? ORDER BY id',
    ...
    $Data{Events} = \@Events;
    $DBObject->Prepare(
        SQL => 'SELECT id, commitment_event_id, action_key, action_type, payload_json, status, attempt_count, lifetime_attempt_count, replay_count, available_time, processed_time, last_error, delivery_ref, response_code FROM careoncloud_escalation_outbox WHERE tenant_id = ? AND commitment_id = ? ORDER BY id',
        Bind => [ \$Param{TenantID}, \$Param{CommitmentID} ],
    );
    my @Escalations;
    while ( my @Row = $DBObject->FetchrowArray() ) {
...
sub _EventAdd {
    ...
    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    my $OK = $DBObject->Do(
        SQL => 'INSERT INTO careoncloud_commitment_event (tenant_id, commitment_id, event_type, event_time, from_status, to_status, consumed_seconds, actor, reason, create_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp)', Bind => \@Bind,
    );
    return if !$OK;
    $DBObject->Prepare(
        SQL => 'SELECT id FROM careoncloud_commitment_event WHERE tenant_id = ? AND commitment_id = ? ORDER BY id DESC',
...
sub _EscalationsQueue {
    ...
    for my $Action (@Actions) {
        ...
        my @Bind = map { \$_ } @Values;
        my $OK = $DBObject->Do(
            SQL => 'INSERT INTO careoncloud_escalation_outbox (tenant_id, commitment_id, commitment_event_id, action_key, action_type, payload_json, status, attempt_count, available_time, last_error, create_time, change_time) '
                . 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp, current_timestamp)', Bind => \@Bind,
        );
        if (!$OK) {
            $DBObject->Prepare(
                SQL => 'SELECT id FROM careoncloud_escalation_outbox WHERE tenant_id = ? AND commitment_event_id = ? AND action_key = ?',
...
sub _RequestExists {
    my ( $Self, %Param ) = @_;
    $Kernel::OM->Get('Kernel::System::DB')->Prepare( SQL => 'SELECT id FROM careoncloud_request WHERE tenant_id = ? AND id = ?', Bind => [ \$Param{TenantID}, \$Param{RequestID} ], Limit => 1 );
...
sub _Enabled { return $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Commitment::Enabled') ? 1 : 0 }
