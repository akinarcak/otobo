...
package Kernel::System::CareOnCloud::Audit;
...
sub Record {
    ...
    if ( length( $Param{SourceIP} // q{} ) ) {
        ...
        my $Salt = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Audit::IPHashSalt') // q{};
    ...
    eval {
        ...
        $DB->Do(
            SQL => 'INSERT IGNORE INTO careoncloud_audit_head (tenant_id, last_sequence, last_hash, version, change_time) VALUES (?, 0, ?, ?, current_timestamp)',
            Bind => [ \$Param{TenantID}, \$Zero, \$Version ],
        );
        $DB->Prepare(
            SQL => 'SELECT last_sequence, last_hash FROM careoncloud_audit_head WHERE tenant_id = ? FOR UPDATE',
            Bind => [ \$Param{TenantID} ],
        );
        my ( $LastSequence, $PreviousHash ) = $DB->FetchrowArray();
        die "HEAD_MISSING\n" if !defined $LastSequence;
        $DB->Prepare(
            SQL => 'SELECT sequence_no, event_uuid, event_hash, previous_hash FROM careoncloud_audit_event WHERE tenant_id = ? AND dedupe_key = ?',
        ...
        die "EVENT_INSERT_FAILED\n" if !$DB->Do(
            SQL => 'INSERT INTO careoncloud_audit_event (tenant_id, sequence_no, event_uuid, event_time, actor_type, actor_id, action_name, object_type, object_id, correlation_id, dedupe_key, from_state, to_state, outcome, source_ip_hash, details_json, previous_hash, event_hash, create_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp)',
            Bind => \@Bind,
        );
        die "HEAD_UPDATE_FAILED\n" if !$DB->Do(
            SQL => 'UPDATE careoncloud_audit_head SET last_sequence = ?, last_hash = ?, version = version + 1, change_time = current_timestamp WHERE tenant_id = ?',
...
sub List {
    ...
    my $Limit = $Param{Limit} // $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Audit::ExportLimit') // 500;
    ...
    my @Bind = map { \$_ } @Values;
    $DB->Prepare(
        SQL => "SELECT sequence_no, event_uuid, event_time, actor_type, actor_id, action_name, object_type, object_id, correlation_id, dedupe_key, from_state, to_state, outcome, source_ip_hash, details_json, previous_hash, event_hash FROM careoncloud_audit_event WHERE tenant_id = ? AND sequence_no > ?$Filter ORDER BY sequence_no",
...
sub Verify {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => 'SELECT sequence_no, event_uuid, event_time, actor_type, actor_id, action_name, object_type, object_id, correlation_id, dedupe_key, from_state, to_state, outcome, source_ip_hash, details_json, previous_hash, event_hash FROM careoncloud_audit_event WHERE tenant_id = ? ORDER BY sequence_no',
        Bind => [ \$Param{TenantID} ],
    );
    ...
    $DB->Prepare( SQL => 'SELECT last_sequence, last_hash FROM careoncloud_audit_head WHERE tenant_id = ?', Bind => [ \$Param{TenantID} ], Limit => 1 );
...
sub _Authorize {
    ...
    if ( ref $Subject ne 'HASH' && $Param{UserID} ) {
        my $Directory = eval { $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory') };
...
sub _Enabled { return $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Audit::Enabled') ? 1 : 0 }
