...
$Helper->ConfigSettingChange(Key=>'CareOnCloud::Audit::IPHashSalt',Value=>'unit-test-secret-salt');
...
for my $Tenant($A,$B){my @V=($Tenant,"Tenant $Tenant",'active',1,$AuditorID,$AuditorID);my @B=map{\$_}@V;$DB->Do(SQL=>'INSERT INTO careoncloud_tenant (key_name,name,status,version,create_time,create_by,change_time,change_by) VALUES (?,?,?, ?,current_timestamp,?,current_timestamp,?)',Bind=>\@B)||die}
for my $Role([$A,$AuditorID,'auditor'],[$B,$AuditorID,'agent']){my($T,$U,$R)=@{$Role};my @V=($T,$U,$R,'active',$AuditorID,$AuditorID);my @B=map{\$_}@V;$DB->Do(SQL=>'INSERT INTO careoncloud_tenant_agent_role (tenant_id,user_id,role_name,status,create_time,create_by,change_time,change_by) VALUES (?,?,?,?,current_timestamp,?,current_timestamp,?)',Bind=>\@B)||die}
my $Audit=$Kernel::OM->Get('Kernel::System::CareOnCloud::Audit');
my $One=$Audit->Record(TenantID=>$A,ActorType=>'customer',ActorID=>'customer:test',Action=>'request.created',ObjectType=>'request',ObjectID=>'101',CorrelationID=>'REQ-101',DedupeKey=>'request:101:created',FromState=>'',ToState=>'awaiting_approval',Outcome=>'success',SourceIP=>'10.1.2.3',Details=>{catalog_item_id=>7});
...
my($EventID);$DB->Prepare(SQL=>'SELECT id FROM careoncloud_audit_event WHERE tenant_id = ? AND sequence_no = 1',Bind=>[\$A]);($EventID)=$DB->FetchrowArray();my($Changed,$ID)=('tampered',$EventID);$DB->Do(SQL=>'UPDATE careoncloud_audit_event SET to_state = ? WHERE id = ?',Bind=>[\$Changed,\$ID]);
...
is($Audit->Record(TenantID=>$A,ActorType=>'agent',ActorID=>'a',Action=>'Bad Action',ObjectType=>'request',ObjectID=>'1',DedupeKey=>'invalid:action',Details=>{})->{Error},'ACTION_INVALID','invalid event vocabulary fails closed');
$Helper->ConfigSettingChange(Key=>'CareOnCloud::Audit::Enabled',Value=>0); is($Audit->List(UserID=>$AuditorID,TenantID=>$A)->{Error},'AUDIT_DISABLED','disabled audit fails closed');
done_testing;
