...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::API::Enabled', Value => 1 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::API::BcryptCost', Value => 9 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::API::TokenTTLMax', Value => 900 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::API::RateLimitMax', Value => 100 );
...
$DB->Prepare( SQL => 'SELECT COUNT(*) FROM careoncloud_api_client WHERE client_id = ?', Bind => [ \$NestedClientID ] );
...
$DB->Prepare( SQL => 'SELECT COUNT(*) FROM careoncloud_audit_event WHERE tenant_id = ? AND dedupe_key = ?', Bind => [ \$Tenant, \$NestedDedupeKey ] );
...
$DB->Prepare( SQL => 'SELECT secret_hash FROM careoncloud_api_client WHERE client_id = ?', Bind => [ \$ClientID ], Limit => 1 );
...
$DB->Prepare( SQL => 'SELECT token_hash FROM careoncloud_api_token WHERE client_id = ?', Bind => [ \$ClientID ], Limit => 1 );
...
$DB->Prepare( SQL => 'SELECT secret_hash, version FROM careoncloud_api_client WHERE client_id = ?', Bind => [ \$ClientID ], Limit => 1 );
...
my $Audit = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->List( Subject => $Admin, TenantID => $Tenant, Limit => 100 );
is(
...
{
    local $Kernel::OM->Get('Kernel::Config')->{'CareOnCloud::API::Enabled'} = 0;
    is( $API->TokenIssue( ClientID => $ClientID, ClientSecret => $NewSecret )->{Error}, 'API_DISABLED', 'disabled API fails closed' );
}

ok( $DB->Do( SQL => 'DELETE FROM careoncloud_api_rate WHERE client_id = ?', Bind => [ \$ClientID ] ), 'rate fixtures removed' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_api_token WHERE client_id = ?', Bind => [ \$ClientID ] ), 'token fixtures removed' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_api_client WHERE client_id = ?', Bind => [ \$ClientID ] ), 'client fixture removed' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_event WHERE tenant_id = ?', Bind => [ \$Tenant ] ), 'API audit fixture removed' );
ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_head WHERE tenant_id = ?', Bind => [ \$Tenant ] ), 'API audit head removed' );
for my $TenantID ( $Tenant, $Other ) {
    ok( $DB->Do( SQL => 'DELETE FROM careoncloud_tenant WHERE key_name = ?', Bind => [ \$TenantID ] ), "tenant fixture $TenantID removed" );
}
