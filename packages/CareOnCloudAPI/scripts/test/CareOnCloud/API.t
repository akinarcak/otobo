...
my $API    = $Kernel::OM->Get('Kernel::System::CareOnCloud::API');
my $Suffix = lc $Helper->GetRandomID();
my $Tenant = "api-read-$Suffix";
my $Other  = "api-hidden-$Suffix";

for my $Setting (
    [ 'CareOnCloud::API::Enabled', 1 ], [ 'CareOnCloud::API::BcryptCost', 9 ],
    [ 'CareOnCloud::API::TokenTTLMax', 900 ], [ 'CareOnCloud::API::RateLimitMax', 100 ],
    [ 'CareOnCloud::Audit::Enabled', 1 ], [ 'CareOnCloud::TicketAudit::Enabled', 1 ],
    [ 'CareOnCloud::TenantGuard::Enabled', 1 ], [ 'CareOnCloud::TicketPolicy::Enabled', 0 ],
) {
...
for my $TenantID ( $Tenant, $Other ) {
    push @TicketIDs, $Ticket->TicketCreate(
        TN => 'CareOnCloudAPI' . $Helper->GetRandomID(), Title => "API ticket $TenantID",
...
for my $TicketID (@TicketIDs) {
    ...
    ok( $DB->Do( SQL => 'DELETE FROM careoncloud_ticket_scope WHERE ticket_id = ?', Bind => [ \$TicketID ] ), "ticket scope $TicketID removed" );
}
for my $TenantID ( $Tenant, $Other ) {
    ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_event WHERE tenant_id = ?', Bind => [ \$TenantID ] ), "audit events $TenantID removed" );
    ok( $DB->Do( SQL => 'DELETE FROM careoncloud_audit_head WHERE tenant_id = ?', Bind => [ \$TenantID ] ), "audit head $TenantID removed" );
    ok( $DB->Do( SQL => 'DELETE FROM careoncloud_tenant WHERE key_name = ?', Bind => [ \$TenantID ] ), "tenant $TenantID removed" );
}
