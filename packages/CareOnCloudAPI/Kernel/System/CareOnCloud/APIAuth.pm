...
# --
package Kernel::System::CareOnCloud::APIAuth;
...
our @ObjectDependencies = (
    'Kernel::Config', 'Kernel::System::CareOnCloud::Audit', 'Kernel::System::CareOnCloud::TenantGuard',
...
sub ClientCreate {
    ...
    my $ClientID = $Param{ClientID} // 'careoncloud_' . $Kernel::OM->Get('Kernel::System::Main')->GenerateRandomString(
    ...
    my $Result; my $OK = eval {
        ...
        die "CLIENT_INSERT_FAILED\n" if !$DB->Do(
            SQL => 'INSERT INTO careoncloud_api_client (client_id, tenant_id, name, secret_hash, role_name, status, token_ttl, rate_limit, version, create_time, create_by, change_time, change_by) '
    ...
    if (!$OK) { my $Failure = $@; eval { $DB->Rollback() } if $OwnTransaction; return $Self->_Error( $Failure =~ /AUDIT/ ? 'AUDIT_WRITE_FAILED' : 'CLIENT_CREATE_FAILED' ) }
    return $Result;
}
...
sub TokenIssue {
    ...
    my $OK = eval {
        ...
        die "TOKEN_INSERT_FAILED\n" if !$DB->Do(
            SQL => "INSERT INTO careoncloud_api_token (token_hash, client_id, expires_at, status, create_time) VALUES (?, ?, DATE_ADD(current_timestamp, INTERVAL ? SECOND), 'active', current_timestamp)",
            Bind => \@Bind,
        );
    ...
    if (!$OK) {
        my $Failure = $@; eval { $DB->Rollback() } if $OwnTransaction;
...
sub ClientSecretRotate {
    ...
    my $OK = eval {
        ...
        die "CLIENT_ROTATE_FAILED\n" if !$DB->Do(
            SQL => "UPDATE careoncloud_api_client SET secret_hash = ?, version = version + 1, change_time = current_timestamp, change_by = ? WHERE client_id = ? AND tenant_id = ? AND status = 'active' AND version = ?",
            Bind => \@Bind,
        );
        ...
        die "TOKEN_REVOKE_FAILED\n" if !$DB->Do(
            SQL => "UPDATE careoncloud_api_token SET status = 'revoked' WHERE client_id = ? AND status = 'active'",
            Bind => [ \$ClientID ],
        );
        my $Audit = $Self->_Audit(
...
sub ClientRevoke {
    ...
    my $OK = eval {
        ...
        die "CLIENT_REVOKE_FAILED\n" if !$DB->Do(
            SQL => "UPDATE careoncloud_api_client SET status = 'revoked', version = version + 1, change_time = current_timestamp, change_by = ? WHERE client_id = ? AND tenant_id = ? AND status = 'active' AND version = ?",
            Bind => \@ClientBind,
        );
        my $Updated = $Self->_ClientGet( ClientID => $Client->{ClientID} );
        die "VERSION_CONFLICT\n"
...
sub TokenValidate {
    ...
    $DB->Prepare(
        SQL => "SELECT c.client_id, c.tenant_id, c.role_name, c.rate_limit, c.status FROM careoncloud_api_token t INNER JOIN careoncloud_api_client c ON c.client_id = t.client_id INNER JOIN careoncloud_tenant d ON d.key_name = c.tenant_id WHERE t.token_hash = ? AND t.status = 'active' AND t.expires_at > current_timestamp AND c.status = 'active' AND d.status = 'active'",
        Bind => [ \$Hash ], Limit => 1,
    );
    my @Row = $DB->FetchrowArray(); return $Self->_Error('TOKEN_INVALID') if !@Row;
    $DB->Do( SQL => 'UPDATE careoncloud_api_token SET last_used_time = current_timestamp WHERE token_hash = ?', Bind => [ \$Hash ] );
...
sub TokenRevoke {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => "SELECT c.client_id, c.tenant_id FROM careoncloud_api_token t INNER JOIN careoncloud_api_client c ON c.client_id = t.client_id WHERE t.token_hash = ? AND t.status = 'active'",
        Bind => [ \$Hash ], Limit => 1,
    );
    ...
    my $OK = eval {
        $DB->BeginWork() if $OwnTransaction;
        die "TOKEN_REVOKE_FAILED\n" if !$DB->Do(
            SQL => "UPDATE careoncloud_api_token SET status = 'revoked' WHERE token_hash = ? AND status = 'active'", Bind => [ \$Hash ],
        );
...
sub _RateConsume {
    ...
    return $Self->_Error('RATE_DATABASE_ERROR') if !$DB->Do(
        SQL => 'INSERT INTO careoncloud_api_rate (client_id, window_start, request_count) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE request_count = request_count + 1', Bind => \@Bind,
    );
    $DB->Prepare( SQL => 'SELECT request_count FROM careoncloud_api_rate WHERE client_id = ? AND window_start = ?', Bind => \@Bind, Limit => 1 );
...
sub _ClientValidate {
    ...
    my $TTLMax = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::API::TokenTTLMax') // 900;
    my $RateMax = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::API::RateLimitMax') // 600;
...
sub _ClientGet {
    ...
    $DB->Prepare( SQL => 'SELECT client_id, tenant_id, secret_hash, role_name, status, token_ttl, rate_limit, version FROM careoncloud_api_client WHERE client_id = ?', Bind => [ \$ID ], Limit => 1 );
...
sub _SecretHash {
    ...
    my $Cost = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::API::BcryptCost') // 12; $Cost = 9 if $Cost < 9; $Cost = 16 if $Cost > 16;
...
sub _TenantActive { my ( $Self, %Param ) = @_; my $ID = $Param{TenantID}; my $DB = $Kernel::OM->Get('Kernel::System::DB'); $DB->Prepare( SQL => "SELECT 1 FROM careoncloud_tenant WHERE key_name = ? AND status = 'active'", Bind => [ \$ID ], Limit => 1 ); my ($OK) = $DB->FetchrowArray(); return $OK }
sub _Audit { my ( $Self, %Param ) = @_; return $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->Record( TenantID => $Param{TenantID}, ActorType => 'integration', ActorID => $Param{ActorID}, Action => $Param{Action}, ObjectType => 'api_client', ObjectID => $Param{ClientID}, CorrelationID => "api-client:$Param{ClientID}", DedupeKey => $Param{DedupeKey}, FromState => q{}, ToState => $Param{ToState}, Outcome => 'success', Details => $Param{Details} ) }
sub _Enabled { return $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::API::Enabled') ? 1 : 0 }
