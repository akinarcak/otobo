...
# --
package Kernel::System::CareOnCloud::API;
...
our @ObjectDependencies = (
    ...
    'Kernel::System::CareOnCloud::APIAuth',
    'Kernel::System::CareOnCloud::Request',
    'Kernel::System::CareOnCloud::Webhook',
    'Kernel::System::DB',
);
...
sub TicketList {
    ...
    my $Prepared = $DB->Prepare(
        SQL => 'SELECT t.id, t.tn, t.title, q.name, ts.name, tp.name, t.customer_id, t.create_time, t.change_time '
            . 'FROM careoncloud_ticket_scope s INNER JOIN ticket t ON t.id = s.ticket_id '
...
sub RequestCreate {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Request')->CustomerSubmit(
...
sub RequestGet {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Request')->CustomerGet(
        CustomerUserID => $Param{RequesterLogin},
        CustomerID     => $TenantID,
...
sub RequestApprovalDecide {
    ...
    return $Authorization if !$Authorization->{Success};
    my $TenantID = $Authorization->{Data}->{TenantID};
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Request')->ApprovalDecide(
        TenantID          => $TenantID,
        RequestID         => $Param{RequestID},
...
sub RequestTaskUpdate {
    ...
    return $Authorization if !$Authorization->{Success};
    my $TenantID = $Authorization->{Data}->{TenantID};
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Request')->TaskUpdate(
        TenantID          => $TenantID,
        TaskID            => $Param{TaskID},
        Status            => $Param{Status},
        ExpectedVersion   => $Param{ExpectedVersion},
...
sub WebhookSubscriptionCreate {
    ...
    return $Authorization if !$Authorization->{Success};
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Webhook')->SubscriptionCreate(
...
sub WebhookSubscriptionList {
    ...
    my $Authorization = $Self->_Authorize( %Param, Action => 'tenant.manage' );
    return $Authorization if !$Authorization->{Success};
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Webhook')->SubscriptionList(
        Subject => $Authorization->{Data}->{Subject}, TenantID => $Authorization->{Data}->{TenantID},
    );
    return $Result if !$Result->{Success};
...
sub WebhookSubscriptionGet {
    ...
    my $Authorization = $Self->_Authorize( %Param, Action => 'tenant.manage' );
    return $Authorization if !$Authorization->{Success};
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Webhook')->SubscriptionGet(
...
sub WebhookSubscriptionUpdate {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::Webhook')->SubscriptionUpdate(
        Subject => $Authorization->{Data}->{Subject}, TenantID => $Authorization->{Data}->{TenantID},
...
sub _Authorize {
    my ( $Self, %Param ) = @_;
    return $Kernel::OM->Get('Kernel::System::CareOnCloud::APIAuth')->Authorize(
