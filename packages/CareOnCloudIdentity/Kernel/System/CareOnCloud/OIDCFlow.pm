...
# --
package Kernel::System::CareOnCloud::OIDCFlow;
...
our $VERSION='0.3.0';
our @ObjectDependencies=qw(Kernel::System::DB Kernel::System::CareOnCloud::Identity Kernel::System::OpenIDConnect::Token);
sub new{return bless{},$_[0]}

sub Begin {
    my($Self,%Param)=@_;
    my $Provider=$Kernel::OM->Get('Kernel::System::CareOnCloud::Identity')->ProviderRouteGetByKey(%Param);
    ...
    return $Self->_Error('FLOW_CREATE_FAILED') if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL=>q{INSERT INTO careoncloud_oidc_flow (state_digest,tenant_id,provider_id,browser_digest,nonce_digest,verifier_digest,return_path,status,expires_at,create_time,change_time) VALUES (?,?,?,?,?,?,?,'pending',DATE_ADD(current_timestamp,INTERVAL 300 SECOND),current_timestamp,current_timestamp)},Bind=>\@B);
...
sub ExchangeContextGet {
    ...
    $DB->Prepare(
        SQL => q{SELECT f.tenant_id,p.key_name,f.browser_digest,f.verifier_digest FROM careoncloud_oidc_flow f INNER JOIN careoncloud_identity_provider p ON p.id=f.provider_id WHERE f.state_digest=? AND f.status='pending' AND f.expires_at>current_timestamp},
        Bind => [ \$StateDigest ],
        Limit => 1,
    );
...
sub CallbackVerify {
    ...
    my $OK = eval {
        ...
        $DB->Prepare(SQL=>q{SELECT f.tenant_id,f.provider_id,f.browser_digest,f.nonce_digest,f.verifier_digest,f.return_path,f.status,p.key_name,p.issuer,p.audience FROM careoncloud_oidc_flow f INNER JOIN careoncloud_identity_provider p ON p.id=f.provider_id WHERE f.state_digest=? AND f.expires_at>current_timestamp FOR UPDATE},Bind=>[\$StateDigest]);
        my @R=$DB->FetchrowArray(); if(!@R){$Result=$Self->_Error('FLOW_NOT_FOUND_OR_EXPIRED')}
            ...
            if(!$Token->{Success}){$Result=$Self->_Error('TOKEN_SIGNATURE_OR_CLAIMS_INVALID')}
                ...
                else{$Result=$Kernel::OM->Get('Kernel::System::CareOnCloud::Identity')->VerifiedClaimsResolve(Verification=>{Verified=>1,Verifier=>'oidc',Issuer=>$R[8],Audience=>$R[9],Claims=>$C});
                    if($Result->{Success}){
                        my $Consumed = $DB->Do(
                            SQL => q{UPDATE careoncloud_oidc_flow SET status='consumed',change_time=current_timestamp WHERE state_digest=? AND status='pending'},
                            Bind => [\$StateDigest],
                        );
