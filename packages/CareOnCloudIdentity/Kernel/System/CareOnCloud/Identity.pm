...
# --
package Kernel::System::CareOnCloud::Identity;
...
our $VERSION = '0.3.0';
our @ObjectDependencies = qw(Kernel::Config Kernel::System::DB Kernel::System::CareOnCloud::Audit Kernel::System::CareOnCloud::TenantGuard Kernel::System::JSON Kernel::System::Log);
...
sub _ProviderCreate {
    ...
    my $OK = $Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => q{INSERT INTO careoncloud_identity_provider (tenant_id,key_name,issuer,audience,allowed_domains_json,group_role_map_json,status,version,create_time,create_by,change_time,change_by) VALUES (?,?,?,?,?,?,'active',1,current_timestamp,?,current_timestamp,?)},
        Bind => \@Bind,
    );
...
sub _VerifiedClaimsResolve {
    my ( $Self, %Param ) = @_;
    return $Self->_Error('IDENTITY_DISABLED') if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Identity::Enabled');
    ...
    $DB->Prepare(
        SQL => 'SELECT id,user_login,email,status,version FROM careoncloud_federated_identity WHERE provider_id = ? AND subject_id = ?',
    ...
    if ($IdentityID) {
        ...
        $DB->Do( SQL => 'UPDATE careoncloud_federated_identity SET last_seen=current_timestamp,change_time=current_timestamp WHERE id=?', Bind => [ \$IdentityID ] );
    }
    else {
        my $TenantID = $Provider->{TenantID};
        $DB->Prepare(
            SQL => 'SELECT id FROM careoncloud_federated_identity WHERE tenant_id=? AND user_login=?',
        ...
        return $Self->_Error('LOGIN_ALREADY_LINKED') if !$DB->Do(
            SQL => q{INSERT INTO careoncloud_federated_identity (tenant_id,provider_id,subject_id,user_login,email,status,version,last_seen,create_time,change_time) VALUES (?,?,?,?,?,'active',1,current_timestamp,current_timestamp,current_timestamp)},
            Bind => \@Bind,
        );
        $DB->Prepare( SQL => 'SELECT id FROM careoncloud_federated_identity WHERE provider_id=? AND subject_id=?', Bind => [ \$ProviderID, \$Subject ], Limit => 1 );
...
sub _ProviderByRoute {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => 'SELECT id,tenant_id,key_name,issuer,audience,allowed_domains_json,group_role_map_json,status,version FROM careoncloud_identity_provider WHERE issuer=? AND audience=?',
...
sub ProviderRouteGetByKey {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => 'SELECT issuer,audience FROM careoncloud_identity_provider WHERE tenant_id=? AND key_name=? AND status=\'active\'',
