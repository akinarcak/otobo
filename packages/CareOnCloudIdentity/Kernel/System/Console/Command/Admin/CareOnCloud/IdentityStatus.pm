...
# --
package Kernel::System::Console::Command::Admin::CareOnCloud::IdentityStatus;
...
sub Run {
    ...
    my %Tables=map {$_=>($T{$_}?1:0)} qw(careoncloud_identity_provider careoncloud_federated_identity careoncloud_oidc_flow);
    my $Enabled=$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Identity::Enabled')?1:0;
    my $Success=$Enabled && !grep {!$Tables{$_}} keys %Tables;
    my $Data={Success=>$Success?1:0,Enabled=>$Enabled,Package=>'CareOnCloudIdentity',Version=>'0.3.0',Tables=>\%Tables};
