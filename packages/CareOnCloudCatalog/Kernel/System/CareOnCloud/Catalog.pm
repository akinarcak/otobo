...
package Kernel::System::CareOnCloud::Catalog;
...
sub _CatalogItemSchemaSet {
    ...
    if (!$Current) {
        ...
        my $Success = $DBObject->Do(
            SQL => 'INSERT INTO careoncloud_catalog_item_schema '
        ...
        my @Bind = map { \$_ } @Values;
        my $Success = $DBObject->Do(
            SQL => 'UPDATE careoncloud_catalog_item_schema SET schema_json = ?, version = version + 1, '
...
sub _List {
    ...
    my $Limit = $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Catalog::ListLimit') || 200;
...
sub _Authorize {
    ...
    return $Self->_Error( Error => 'CATALOG_DISABLED' )
        if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Catalog::Enabled');
...
sub _SchemaRowGet {
    ...
    $DBObject->Prepare(
        SQL => 'SELECT catalog_item_id, tenant_id, schema_json, version, create_time, create_by, change_time, change_by '
            . 'FROM careoncloud_catalog_item_schema WHERE catalog_item_id = ? AND tenant_id = ?',
...
sub _MetaGet {
    ...
    my %Meta = (
        ...
        Offering => {
            Table       => 'careoncloud_service_offering',
            IDAPI       => 'OfferingID',
            ParentAPI   => 'ServiceID',
            ParentDB    => 'service_id',
            ParentTable => 'careoncloud_service',
        ...
        CatalogItem => {
            Table       => 'careoncloud_catalog_item',
            IDAPI       => 'CatalogItemID',
            ParentAPI   => 'OfferingID',
            ParentDB    => 'offering_id',
            ParentTable => 'careoncloud_service_offering',
            TypeAPI     => 'RequestType',
            TypeDB      => 'request_type',
