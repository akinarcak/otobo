...
# --
package Kernel::System::CareOnCloud::CatalogConstraint;
...
my @Constraint = (
    { Name => 'careoncloud_fk_offering_service_tenant', Table => 'careoncloud_service_offering', Columns => [qw(tenant_id service_id)], ForeignTable => 'careoncloud_service', ForeignColumns => [qw(tenant_id id)] },
    { Name => 'careoncloud_fk_item_offering_tenant', Table => 'careoncloud_catalog_item', Columns => [qw(tenant_id offering_id)], ForeignTable => 'careoncloud_service_offering', ForeignColumns => [qw(tenant_id id)] },
    { Name => 'careoncloud_fk_schema_item_tenant', Table => 'careoncloud_catalog_item_schema', Columns => [qw(tenant_id catalog_item_id)], ForeignTable => 'careoncloud_catalog_item', ForeignColumns => [qw(tenant_id id)] },
);
...
sub Ensure {
    ...
    if ( !$Status->{UniqueItemTenantID} ) {
        return { Success => 0, Error => 'UNIQUE_INDEX_CREATE_FAILED' } if !$DB->Do(
            SQL => 'ALTER TABLE careoncloud_catalog_item ADD UNIQUE INDEX careoncloud_item_tenant_id (tenant_id, id)',
        );
    }
...
sub StatusGet {
    ...
    $DB->Prepare(
        SQL => 'SELECT constraint_name, table_name, column_name, referenced_table_name, referenced_column_name, ordinal_position '
            ...
            . q{AND table_name IN ('careoncloud_service_offering','careoncloud_catalog_item','careoncloud_catalog_item_schema') }
    ...
    $DB->Prepare(
        SQL => q{SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() }
            . q{AND table_name = 'careoncloud_catalog_item' AND index_name = 'careoncloud_item_tenant_id' AND non_unique = 0},
    );
