...
my $Policy = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketPolicy');
my $Suffix = lc $Helper->GetRandomID();
my $TenantA = "policy-a-$Suffix";
my $TenantB = "policy-b-$Suffix";

$Helper->ConfigSettingChange( Key => 'CareOnCloud::TicketPolicy::Enabled', Value => 0 );
...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::TenantGuard::AllowPlatformAdmin', Value => 0 );

for my $TenantID ( $TenantA, $TenantB ) {
    my @Values = ( $TenantID, "Policy $TenantID", 1, 1 );
    my @Bind = map { \$_ } @Values;
    ok( $DB->Do(
        SQL => "INSERT INTO careoncloud_tenant (key_name, name, status, version, create_time, create_by, change_time, change_by) "
...
ok( $DB->Do(
    SQL => "INSERT INTO careoncloud_tenant_agent_role (tenant_id, user_id, role_name, status, version, create_time, create_by, change_time, change_by) "
        . "VALUES (?, ?, ?, 'active', 1, current_timestamp, ?, current_timestamp, ?)",
    Bind => \@RoleBindA,
...
for my $TenantID ( $TenantA, $TenantB ) {
    push @TicketIDs, $Ticket->TicketCreate(
        TN => 'CareOnCloudPOL' . $Helper->GetRandomID(), Title => "Policy ticket $TenantID",
...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::TicketPolicy::Enabled', Value => 1 );
...
my $GICommon = bless {}, 'Kernel::GenericInterface::Operation::Ticket::Common';
{
    ...
    sub CareOnCloudPolicyTestCall { return $_[0]->CheckAccessPermissions( %{ $_[1] } ) }
...
ok(
    Kernel::GenericInterface::Operation::Ticket::TicketGet::CareOnCloudPolicyTestCall( $GICommon, $GICall ),
    'TicketGet caller is mapped to its operation-specific action',
);
ok(
    Kernel::GenericInterface::Operation::Ticket::TicketHistoryGet::CareOnCloudPolicyTestCall( $GICommon, $GICall ),
    'TicketHistoryGet caller is mapped to its operation-specific action',
);
ok(
    Kernel::GenericInterface::Operation::Ticket::TicketUpdate::CareOnCloudPolicyTestCall( $GICommon, $GICall ),
    'TicketUpdate caller is mapped to its operation-specific action',
);
ok(
    $GICommon->CheckAccessPermissions( UserID => 1, UserType => 'User', TicketID => $TicketIDs[0], CareOnCloudAction => 'integration.ticket.get' ),
    'Generic Interface common adapter permits core-authorized same-tenant read',
);
ok(
    !$GICommon->CheckAccessPermissions( UserID => 1, UserType => 'User', TicketID => $TicketIDs[1], CareOnCloudAction => 'integration.ticket.get' ),
    'Generic Interface common adapter denies cross-tenant get/history/update access',
);
ok(
    $GICommon->CheckAccessPermissions( UserID => 1, UserType => 'User', TicketID => $TicketIDs[0], CareOnCloudAction => 'integration.ticket.update' ),
    'agent role permits Generic Interface update action',
);
ok(
    !$GICommon->CheckAccessPermissions( UserID => 1, UserType => 'User', TicketID => $TicketIDs[0], CareOnCloudAction => 'integration.ticket.unknown' ),
