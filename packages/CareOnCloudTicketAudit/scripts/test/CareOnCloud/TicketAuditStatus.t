...
my $Command = $Kernel::OM->Get('Kernel::System::Console::Command::Admin::CareOnCloud::TicketAuditStatus');
...
is( $Status->{Package}, 'CareOnCloudTicketAudit', 'status identifies package' );
is( $Status->{Version}, '0.8.19', 'status identifies current package version' );
...
ok( $Status->{Tables}->{careoncloud_ticket_scope}, 'ticket scope table is reported' );
is( $Status->{Counts}->{UnboundTickets}, 0, 'no legacy ticket is left outside tenant scope' );
is( $Status->{Counts}->{InvalidTenantTickets}, 0, 'every core ticket maps to an active CareOnCloud tenant' );
