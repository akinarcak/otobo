...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::SearchPolicy::Enabled', Value => 1 );
$Helper->ConfigSettingChange( Key => 'CareOnCloud::SearchPolicy::TenantSafeIndexes', Value => ['ticket'] );
...
ok( $DB->Do(
    SQL => "INSERT INTO careoncloud_tenant_agent_role (tenant_id, user_id, role_name, status, version, create_time, create_by, change_time, change_by) VALUES (?, ?, ?, 'active', 1, current_timestamp, ?, current_timestamp, ?)",
    Bind => \@RoleBind,
...
$Helper->ConfigSettingChange( Key => 'CareOnCloud::SearchPolicy::Enabled', Value => 0 );
