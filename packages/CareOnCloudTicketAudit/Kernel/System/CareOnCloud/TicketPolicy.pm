...
# --
package Kernel::System::CareOnCloud::TicketPolicy;
...
sub TicketAccessCheck {
    ...
    $DB->Prepare(
        SQL => "SELECT s.tenant_id FROM careoncloud_ticket_scope s INNER JOIN careoncloud_tenant t ON t.key_name = s.tenant_id "
...
sub ContextResolve {
    ...
    if ( $Param{UserID} ) {
        my $Context = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory')->ContextGet(
            UserID => $Param{UserID},
        );
        return $Self->_Deny( $Context->{Error} // 'AGENT_CONTEXT_DENIED' ) if !$Context->{Success};
        my $Scope = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantGuard')->ScopeGet(
...
sub AutomationScopeRun {
    ...
    my $Authorization = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantGuard')->AutomationAuthorize(
...
sub _TenantActive {
    ...
    my $DB = $Kernel::OM->Get('Kernel::System::DB');
    $DB->Prepare(
        SQL => "SELECT 1 FROM careoncloud_tenant WHERE key_name = ? AND status = 'active'",
...
sub _Deny {
    ...
    $Kernel::OM->Get('Kernel::System::Log')->Log(
        Priority => 'error', Message => "CareOnCloud_TICKET_POLICY_DENY Reason=$Reason",
    );
