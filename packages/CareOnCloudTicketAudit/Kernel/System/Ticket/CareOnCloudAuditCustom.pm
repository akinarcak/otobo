...
# --
package Kernel::System::Ticket::CareOnCloudAuditCustom;
...
our $VERSION = '0.8.19';
our $CareOnCloudSearchContext;
our $CareOnCloudTicketAuditMergeSuppress;
...

{
    ...
    *Kernel::System::Ticket::TicketCreate = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalTicketCreate->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->TicketCreateRun(
    ...
    *Kernel::System::Ticket::TicketDelete = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalTicketDelete->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->TicketDeleteRun(
    ...
    *Kernel::System::Ticket::TicketMerge = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalTicketMerge->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->TicketMergeRun(
    ...
    *Kernel::System::Ticket::TicketSearch = sub {
        ...
        return $OriginalTicketSearch->( $Self, %Param )
            if $Self->{CareOnCloudTicketPolicySuppress}
            || !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketPolicy::Enabled');

        my $Policy = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketPolicy')->SearchScopeApply(
            Param => \%Param,
        );
    ...
    *Kernel::System::Elasticsearch::TicketSearch = sub {
        my ( $Self, %Param ) = @_;
        my $Context = $Kernel::OM->Get('Kernel::System::CareOnCloud::SearchPolicy')->ContextCreate(%Param);
        return if !$Context->{Success};
        my $Scoped = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketPolicy')->SearchScopeApply(
            Param => \%Param,
        );
        return if !$Scoped->{Success};
        local $CareOnCloudSearchContext = $Context;
        return $OriginalESSearch->( $Self, %{ $Scoped->{Param} } );
    };

    *Kernel::GenericInterface::Invoker::Elasticsearch::Search::PrepareRequest = sub {
        my ( $Self, %Param ) = @_;
        my $Filtered = $Kernel::OM->Get('Kernel::System::CareOnCloud::SearchPolicy')->RequestFilterApply(
            Data => $Param{Data}, Context => $CareOnCloudSearchContext,
        );
        return {
            Success => 0, ErrorMessage => 'CareOnCloud Elasticsearch search denied: ' . ( $Filtered->{Reason} // 'UNKNOWN' ), Data => {},
        } if !$Filtered->{Success};
    ...
    my $Wrap = sub {
        ...
        *{"Kernel::System::Ticket::$Method"} = sub {
            ...
            return $Original->( $Self, %Param )
                if $Self->{CareOnCloudTicketAuditSuppress} || $CareOnCloudTicketAuditMergeSuppress;
            return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->MutationRun(
    ...
    *Kernel::System::Ticket::Article::Backend::MIMEBase::ArticleCreate = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalArticleCreate->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress} || $CareOnCloudTicketAuditMergeSuppress;
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ArticleCreateRun(
    ...
    *Kernel::System::Ticket::Article::Backend::Chat::ArticleCreate = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalChatArticleCreate->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ChatArticleCreateRun(
    ...
    *Kernel::System::Ticket::Article::Backend::Chat::ArticleUpdate = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalChatArticleUpdate->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ChatArticleUpdateRun(
    ...
    *Kernel::System::Ticket::Article::Backend::Chat::ArticleDelete = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalChatArticleDelete->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ChatArticleDeleteRun(
    ...
    *Kernel::System::Ticket::Article::Backend::Invalid::ArticleDelete = sub {
        my ( $Self, %Param ) = @_;
        return $OriginalInvalidArticleDelete->( $Self, %Param ) if $Self->{CareOnCloudTicketAuditSuppress};
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->InvalidArticleDeleteRun(
    ...
    *Kernel::GenericInterface::Operation::Ticket::Common::CheckAccessPermissions = sub {
        ...
        return $CoreAccess if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketPolicy::Enabled');

        my %Identity = $Param{UserType} eq 'Customer'
            ? ( CustomerUserID => $Param{UserID} )
            : ( UserID => $Param{UserID} );
        my $Action = $Param{CareOnCloudAction};
        if ( !defined $Action ) {
            my %OperationAction = (
        ...
        my $Policy = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketPolicy')->TicketAccessCheck(
    ...
    *Kernel::GenericInterface::Operation::Ticket::TicketUpdate::Run = sub {
        ...
        return $OriginalGITicketUpdateRun->( $Self, %Param )
            if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketAudit::Enabled');
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->GenericInterfaceTicketUpdateRun(
    ...
    *Kernel::GenericInterface::Operation::Ticket::TicketCreate::Run = sub {
        ...
        return $OriginalGITicketCreateRun->( $Self, %Param )
            if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketAudit::Enabled');
        return $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->GenericInterfaceTicketCreateRun(
    ...
    *Kernel::System::Console::Command::Maint::Ticket::PendingCheck::Run = sub {
        ...
        return $OriginalPendingCheckRun->( $Self, %Param )
            if !$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketPolicy::Enabled');

        my $DB = $Kernel::OM->Get('Kernel::System::DB');
        return $Self->ExitCodeError() if !$DB->Prepare(
            SQL => "SELECT key_name FROM careoncloud_tenant WHERE status = 'active' ORDER BY key_name",
        );
        my @TenantIDs;
        while ( my @Row = $DB->FetchrowArray() ) { push @TenantIDs, $Row[0] }
        for my $TenantID (@TenantIDs) {
            my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketPolicy')->AutomationScopeRun(
                ...
                Code => sub {
                    ...
                    for my $TicketID (@TicketIDs) {
                        my %Ticket = $TicketObject->TicketGet( TicketID => $TicketID, DynamicFields => 0, UserID => 1 );
                        my $Reconcile = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->SchedulerPendingCheckReconcile(
