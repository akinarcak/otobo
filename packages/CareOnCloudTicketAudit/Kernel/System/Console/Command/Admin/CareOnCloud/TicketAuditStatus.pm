...
# --
package Kernel::System::Console::Command::Admin::CareOnCloud::TicketAuditStatus;
...
sub Run {
    ...
    my $Table = $Existing{careoncloud_ticket_scope} ? 1 : 0;
    my ( $Tickets, $Tenants, $CoreTickets, $Unbound, $Backfillable ) = ( 0, 0, 0, 0, 0 );
    my @UnboundList;
    if ($Table) {
        $DB->Prepare( SQL => 'SELECT COUNT(*), COUNT(DISTINCT tenant_id) FROM careoncloud_ticket_scope' );
        ( $Tickets, $Tenants ) = $DB->FetchrowArray();
        $DB->Prepare( SQL => 'SELECT COUNT(*) FROM ticket' );
        ($CoreTickets) = $DB->FetchrowArray();
        $DB->Prepare( SQL => 'SELECT COUNT(*) FROM ticket t LEFT JOIN careoncloud_ticket_scope s ON s.ticket_id = t.id WHERE s.ticket_id IS NULL' );
        ($Unbound) = $DB->FetchrowArray();
        $DB->Prepare(
            SQL => "SELECT COUNT(*) FROM ticket t JOIN careoncloud_tenant d ON d.key_name = t.customer_id AND d.status = 'active' LEFT JOIN careoncloud_ticket_scope s ON s.ticket_id = t.id WHERE s.ticket_id IS NULL",
        );
        ($Backfillable) = $DB->FetchrowArray();
        $DB->Prepare(
            SQL => 'SELECT t.id, t.tn, t.customer_id FROM ticket t LEFT JOIN careoncloud_ticket_scope s ON s.ticket_id = t.id WHERE s.ticket_id IS NULL ORDER BY t.id', Limit => 100,
        );
    ...
    my $SearchEnabled = $Config->Get('CareOnCloud::SearchPolicy::Enabled') ? 1 : 0;
    my $SafeIndexes = $Config->Get('CareOnCloud::SearchPolicy::TenantSafeIndexes') // [];
    ...
    my $Status = {
        ...
        Package => 'CareOnCloudTicketAudit', Version => $VERSION,
        Enabled => $Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::TicketAudit::Enabled') ? 1 : 0,
        Tables => { careoncloud_ticket_scope => $Table }, Counts => {
    ...
    else { $Self->Print("CareOnCloud TicketAudit $VERSION: (CareOnCloudTicketAudit $VERSION: " . ( $Status->{Success} ? 'OK' : 'FAILED' ) . ")\n") }
