...
# --
package Kernel::Modules::PublicD724Metrics;
...
sub Run {
    ...
    return $Self->_Error( Code => 503, Message => 'metrics unavailable' )
        if !$Config->Get('D724::Observability::Enabled');
    my $Expected = lc( $Config->Get('D724::Observability::MetricsTokenSHA256') // q{} );
    return $Self->_Error( Code => 503, Message => 'metrics unavailable' )
...
sub _Error {
    ...
    $Response->Header( 'Cache-Control' => 'no-store' );
    $Response->Header( 'X-Content-Type-Options' => 'nosniff' );
    $Response->Header( 'WWW-Authenticate' => 'Bearer realm="d724-metrics"' ) if $Param{Authenticate};
