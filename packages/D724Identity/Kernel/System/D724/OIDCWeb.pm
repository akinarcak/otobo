...
# --
package Kernel::System::D724::OIDCWeb;
...
our @ObjectDependencies = qw(
    ...
    Kernel::System::DB
    Kernel::System::D724::Identity
    Kernel::System::D724::OIDCFlow
...
sub Start {
    ...
    my $Flow = $Kernel::OM->Get('Kernel::System::D724::OIDCFlow')->Begin(
    ...
    return {
        ...
        Data    => {
            RedirectURL => $URL,
            State       => $Flow->{Data}->{State},
            CookieName  => 'D724OIDC-' . $Flow->{Data}->{State},
...
sub Callback {
    ...
    my $Exchange = $Kernel::OM->Get('Kernel::System::D724::OIDCFlow')->ExchangeContextGet(
        State          => $Param{State},
        BrowserBinding => $Param{BrowserBinding},
    ...
    my $Verified = $Kernel::OM->Get('Kernel::System::D724::OIDCFlow')->CallbackVerify(
        State          => $Param{State},
        BrowserBinding => $Param{BrowserBinding},
        CodeVerifier   => $Param{CodeVerifier},
        IDToken        => $IDToken,
    ...
    if ( $Param{Surface} eq 'agent' ) {
        ...
        $DB->Prepare(
            SQL => q{SELECT id FROM d724_tenant_agent_role WHERE tenant_id=? AND user_id=? AND status='active'},
...
sub _ProviderContextGet {
    my ( $Self, %Param ) = @_;
    my $Provider = $Kernel::OM->Get('Kernel::System::D724::Identity')->ProviderRouteGetByKey(
    ...
    my $Redirect = $Kernel::OM->Get('Kernel::Config')->Get('D724::Identity::OIDC::RedirectURI') // {};
    ...
    my $Metadata = $Kernel::OM->Get('Kernel::System::D724::OIDCFlow')->MetadataValidate(
        Provider => $Provider,
