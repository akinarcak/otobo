...
# --
package Kernel::System::D724::OIDCAuthBackend;
...
sub new {
    ...
    my $FallbackClass = $Kernel::OM->Get('Kernel::Config')->Get(
        $Surface eq 'agent'
            ? 'D724::Identity::FallbackAuthModule::Agent'
            : 'D724::Identity::FallbackAuthModule::Customer'
    ) || $Type->_DBClass();
    die "Recursive CareOnCloud authentication fallback\n"
        if $FallbackClass =~ m{\AD724|::D724OpenIDConnect\z}smx;
...
sub GetOption {
    ...
    return 1
        if ( $Request->GetParam( Param => 'D724SSO' ) // q{} ) eq '1'
...
sub Auth {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::D724::OIDCWeb')->Callback(
