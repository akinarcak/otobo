...
package Kernel::Modules::AdminD724Catalog;
...
sub Run {
    ...
    if ( $Subaction =~ m{\A(?:ServiceSave|OfferingSave|ItemSave|SchemaSave)\z}smx ) {
        ...
        if ( $Result->{Success} ) {
            return $Layout->Redirect( OP => "Action=AdminD724Catalog;TenantID=$TenantID" );
        }
    ...
    my $Output = $Layout->Header( Title => 'D724 Service Catalog' );
    $Output .= $Layout->NavigationBar();
    $Output .= $Layout->Notify( Priority => 'Error', Info => $Param{Error} ) if $Param{Error};
    $Output .= $Layout->Output( TemplateFile => 'AdminD724Catalog', Data => { TenantID => $TenantID } );
