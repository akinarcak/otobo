...
sub _Result {
    ...
    my $Response=$Kernel::OM->Get('Kernel::System::Web::Response');my $Code=$P{SuccessCode}//200;$Response->Code($Code);$Response->Header('Cache-Control'=>'no-store');$Response->Header('X-Content-Type-Options'=>'nosniff');return q{} if$P{NoContent};my $D=$R->{Data};$Response->Header('Content-Type'=>'application/scim+json;charset=UTF-8');if(ref$D eq'HASH'&&ref$D->{meta}eq'HASH'){$Response->Header('ETag'=>$D->{meta}->{version});$Response->Header('Location'=>'/otobo/scim/v2/'.($P{ResourcePath}//q{}).'/'.$D->{id})if$P{ResourcePath}}return$Kernel::OM->Get('Kernel::System::JSON')->Encode(Data=>$D,SortKeys=>1)
}
