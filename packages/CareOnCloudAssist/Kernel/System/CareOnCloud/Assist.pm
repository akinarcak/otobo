...
# --
package Kernel::System::CareOnCloud::Assist;
use v5.24;use strict;use warnings;use utf8;
our $VERSION='0.1.1';
our @ObjectDependencies=('Kernel::Config','Kernel::System::CareOnCloud::TenantDirectory','Kernel::System::CareOnCloud::TenantGuard','Kernel::System::DB','Kernel::System::JSON');
sub new{return bless{},$_[0]}
sub Suggest{
 my($S,%P)=@_;return$S->_Error('ASSIST_DISABLED')if!$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Assist::Enabled');
 ...
 my$Subject=$P{Subject};if(ref$Subject ne'HASH'&&$P{UserID}){my$C=$Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory')->ContextGet(UserID=>$P{UserID});return$C if!$C->{Success};$Subject=$C->{Subject}}
 my$D=$Kernel::OM->Get('Kernel::System::CareOnCloud::TenantGuard')->DecisionGet(Subject=>$Subject,Resource=>{TenantID=>$P{TenantID}},Action=>'case.read');return$S->_Error('FORBIDDEN',$D->{Reason})if!$D->{Allowed};
 my%QT=$S->_Tokens($Q);return$S->_Error('QUERY_TOO_GENERIC')if!keys%QT;
 my$T=$P{TenantID};my$Max=$Kernel::OM->Get('Kernel::Config')->Get('CareOnCloud::Assist::CandidateLimit')//500;return$S->_Error('CANDIDATE_LIMIT_INVALID')if$Max!~m{\A[1-9][0-9]{1,3}\z}smx;
 my$DB=$Kernel::OM->Get('Kernel::System::DB');$DB->Prepare(SQL=>"SELECT r.id,r.request_number,r.catalog_item_id,c.name,r.status,r.answers_json,r.change_time FROM careoncloud_request r INNER JOIN careoncloud_catalog_item c ON c.tenant_id=r.tenant_id AND c.id=r.catalog_item_id WHERE r.tenant_id=? AND r.status='fulfilled' ORDER BY r.id DESC",Bind=>[\$T],Limit=>$Max);
 my@R;while(my($ID,$Number,$ItemID,$Item,$Status,$Answers,$Changed)=$DB->FetchrowArray()){
  ...
  my$RID=$ID;$DB->Prepare(SQL=>"SELECT result_comment FROM careoncloud_request_task WHERE tenant_id=? AND request_id=? AND status='completed' AND result_comment<>'' ORDER BY id",Bind=>[\$T,\$RID],Limit=>3);my@Notes;while(my($N)=$DB->FetchrowArray()){$N=substr($N,0,500);push@Notes,$N}
