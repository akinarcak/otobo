...
package Kernel::System::MigrateFromOTRS::CareOnCloudItsmTablesMigrate;
...
Kernel::System::MigrateFromOTRS::CareOnCloudItsmTablesMigrate - Migrate ITSM tables to CareOnCloud ESM.
...
sub Run {
    ...
    $CacheObject->Set(
        ...
        Key   => 'MigrationState',
        Value => {
            Task      => 'CareOnCloud ESMItsmTablesMigrate',
            SubTask   => "Migrate ITSM tables to CareOnCloud ESM.",
            StartTime => $Epoch,
        },
    );
    ...
    if ( !$Self->TableExists( Table => $Table ) ) {
        return {
            Message    => $Message,
            Comment    => $Self->{LanguageObject}->Translate( "Nothing to do, as the table '%s' does not exist.", $Table ),
            Successful => 1,
        };
    }
