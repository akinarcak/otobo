...
print STDOUT "\nManage the CareOnCloud ESM daemon process.\n\n";

local $Kernel::OM = Kernel::System::ObjectManager->new(
    'Kernel::System::Log' => {
        LogPrefix => 'CareOnCloud-careoncloud.Daemon.pl',
    },
);
...
Error: You cannot run careoncloud.Daemon.pl as root. Please run it as the 'careoncloud' user or with the help of su:
  su -c "bin/careoncloud.Daemon.pl ..." -s /bin/bash careoncloud
END_MSG
...
sub PrintUsage {
    my $UsageText = "Usage:\n";
    $UsageText .= " careoncloud.Daemon.pl action [--debug] [--force]\n";
    $UsageText .= "\nOptions:\n";
    ...
    $UsageText .= "\n careoncloud.Daemon.pl start --debug SchedulerTaskWorker SchedulerCronTaskManager\n\n";
    $UsageText
        .= "Forced stop reduces the time the main daemon waits other daemons to stop from normal 30 seconds to 5.\n";
    $UsageText .= "\n careoncloud.Daemon.pl stop --force\n";
...
sub Start {
    ...
    while ($DaemonChecker) {
        ...
        if ( !$Kernel::OM->Get('Kernel::Config')->Get('SecureMode') ) {
            $DaemonChecker = 0;
            say STDOUT "Stopping the CareOnCloud ESM Daemon as SecureMode is not activated.";
        ...
        for my $Module ( sort keys %DaemonModules ) {
            ...
            if ( !$ChildPID ) {
                ...
                local $Kernel::OM = Kernel::System::ObjectManager->new(
                    'Kernel::System::Log' => {
                        LogPrefix => "CareOnCloud-careoncloud.Daemon.pl - Daemon $Module",
                    },
                );
