...
careoncloud.psgi - CareOnCloud ESM PSGI application

=head1 SYNOPSIS

    # using the default webserver
    plackup bin/psgi-bin/careoncloud.psgi

    # using the webserver Gazelle
    plackup --server Gazelle bin/psgi-bin/careoncloud.psgi

    # new process for every request , useful for development
    plackup --server Shotgun bin/psgi-bin/careoncloud.psgi
...
in F<careoncloud.web.dockerfile>.

    cp cpanfile.docker cpanfile
    cpanm --local-lib local Carton
    PERL_CPANM_OPT="--local-lib /opt/careoncloud_install/local" carton install
...
# expect that careoncloud.psgi is two levels below the CareOnCloud ESM home dir
...
# The CareOnCloud ESM home is determined from the location of careoncloud.psgi.
...
my $SetPSGIEnvMiddleware = sub {
    ...
    return sub {
        ...
        $Env->{SERVER_SOFTWARE} //= 'careoncloud.psgi';
...
# $Env->{SCRIPT_NAME} contains the matching mountpoint. Can be e.g. '/careoncloud' or '/careoncloud/index.pl'
...
# managed by the CareOnCloud ESM object manager. Inversely this means that applications that follow this middleware
...
my $FixFCGIProxyMiddleware = sub {
    ...
    return sub {
        ...
        # This means that careoncloud.psgi expects that SCRIPT_NAME is either '' or '/' and that
        # PATH_INFO is something like '/careoncloud/index.pl'.
        # But we get PATH_INFO = '' and SCRIPT_NAME = '/careoncloud/index.pl'.
...
my $SyncFromS3Middleware = sub {
    ...
    return sub {
        ...
        # /opt/careoncloud/var/httpd/htdocs/skins/Customer/css-cache
...
# Handler for 'careoncloud', 'careoncloud/', and nested CareOnCloud ESM paths.
...
my $RedirectCareOnCloudApp = sub {
    ...
    # we end up in the default interface. All is fine as long we don't walk up too high, i.e. above 'careoncloud/'.
    my $Redirect;
    if ( $Env->{PATH_INFO} eq '' ) {

        # Special case for https://example.com/careoncloud . The path below 'careoncloud' is empty. So for redirecting
        # we needed information how we got here. Often REQUEST_URI is '/careoncloud' but this is not guaranteed.
        my ($LastComponent) = reverse split /\//, $Env->{REQUEST_URI};
        $Redirect = join '/', $LastComponent, $Interface;    # e.g. careoncloud/index.pl
    }
    else {
...
my $CheckPublicInterfaceMiddleware = sub {
    ...
    return sub {
        ...
        # trick $RedirectCareOnCloudApp into doing the right thing
        $Env->{PATH_INFO} = 'public.pl';

        return $RedirectCareOnCloudApp->($Env);
    };
};
...
my $CareOnCloudApp = builder {
    ...
    # The actual functionality of CareOnCloud ESM is implemented as a collection
    # of Plack middlewares and Plack apps. These components are tied together via an URL map.
    # The CareOnCloud ESM interface modules are actually Plack components, calling to_app() on them
...
builder {
    ...
    # note that 404 below /careoncloud/ already redirects to Frontend::DefaultInterface
    ...
    mount '/careoncloud' => $CareOnCloudApp;
