...
eval {
    ...
    while (1) {
        $DB->Prepare(
            SQL => "SELECT o.id, o.tenant_id, s.status, s.key_name FROM careoncloud_escalation_outbox o LEFT JOIN careoncloud_webhook_subscription s ON s.tenant_id = o.tenant_id AND CONCAT('webhooksub', s.id) = o.action_key WHERE o.status IN ('pending', 'retry') ORDER BY o.available_time, o.id",
            Limit => 1,
        );
    ...
    $DB->Prepare(
        SQL => "SELECT id, status, payload_json FROM careoncloud_escalation_outbox WHERE tenant_id = ? AND commitment_id = 0 AND action_key = ? ORDER BY id DESC",
    ...
    $DB->Prepare(SQL => "SELECT COUNT(*) FROM careoncloud_escalation_outbox WHERE status IN ('pending', 'retry')");
