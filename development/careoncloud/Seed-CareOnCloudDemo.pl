#!/usr/bin/env perl
# --
# CareOnCloud ESM is an enterprise service management platform based on CareOnCloud ESM.
...
die "CareOnCloud_DEMO_CUSTOMER_PASSWORD is required\n" if !length( $ENV{CareOnCloud_DEMO_CUSTOMER_PASSWORD} // q{} );
...
if (!%Company) {
    my $Created = $CompanyObject->CustomerCompanyAdd(
        CustomerID             => $TenantID,
        CustomerCompanyName    => 'CareOnCloud Demo Company',
        ...
        CustomerCompanyComment => 'CareOnCloud isolated demo tenant',
        ValidID                => 1,
        UserID                 => $UserID,
    );
...
die "Could not set demo customer password\n" if !$CustomerUserObject->SetPassword(
    UserLogin => $Login,
    PW        => $ENV{CareOnCloud_DEMO_CUSTOMER_PASSWORD},
);
...
my $Commitment = eval { $Kernel::OM->Get('Kernel::System::CareOnCloud::Commitment') };
if ($Commitment) {
...
say "CareOnCloud demo catalog and commitment policy seeded for tenant $TenantID and customer login $Login.";
