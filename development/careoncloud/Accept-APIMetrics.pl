...
sub MetricCount {
    ...
    $DB->Prepare(
        SQL => "SELECT COALESCE(SUM(request_count),0) FROM careoncloud_api_metric WHERE tenant_id = ? AND route_key = ? AND status_code = ? AND window_start = DATE_FORMAT(current_timestamp, '%Y-%m-%d %H:%i:00')",
...
eval {
    ...
    $DB->Prepare( SQL => "SELECT COUNT(*) FROM careoncloud_api_metric WHERE route_key LIKE '%/%' OR route_key REGEXP '[0-9]{4,}'" );
    my ($HighCardinality) = $DB->FetchrowArray();
    die "High-cardinality raw route label persisted\n" if $HighCardinality;
    $DB->Prepare(
        SQL => "SELECT COALESCE(SUM(request_count),0), COALESCE(SUM(duration_sum_ms),0), COALESCE(MAX(duration_max_ms),0) FROM careoncloud_api_metric WHERE tenant_id = ? AND route_key = 'tickets' AND status_code = 200 AND window_start = DATE_FORMAT(current_timestamp, '%Y-%m-%d %H:%i:00')",
        Bind => [ \$TenantID ],
    );
    my ( $Count, $DurationSum, $DurationMax ) = $DB->FetchrowArray();
    die "Latency aggregate is invalid\n" if $Count < 3 || $DurationSum < 0 || $DurationMax < 0;

    my $Status = $Kernel::OM->Get('Kernel::System::Console::Command::Admin::CareOnCloud::APIStatus')->StatusData();
