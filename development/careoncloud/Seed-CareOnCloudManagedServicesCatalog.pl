...
my @Tenant = @ARGV ? @ARGV : qw(careoncloud-demo showcase-bank showcase-fashion showcase-retail);
my @Category = (
...
sub TenantExists {
    my ($TenantID) = @_;
    $DB->Prepare( SQL => 'SELECT 1 FROM careoncloud_tenant WHERE key_name = ?', Bind => [ \$TenantID ], Limit => 1 );
