...
my $Password = $ENV{CareOnCloud_GI_ACCEPTANCE_PASSWORD} // q{};
die "CareOnCloud_GI_ACCEPTANCE_PASSWORD is required\n" if length $Password < 16;
...
my $ServiceName = 'CareOnCloudAcceptanceTicketLifecycle';
...
my $Bootstrap = $Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory')->Bootstrap(
...
my $TicketID = $Ticket->TicketCreate(
    TN => 'CareOnCloudGI' . time(), Title => 'Generic Interface acceptance before update',
...
my $Before = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ScopeGet( TicketID => $TicketID );
...
$ServiceID = $Webservice->WebserviceAdd(
    ...
    Config => {
        ...
        Provider => {
            Operation => {
                TicketCreate => {
                    Description => 'Ephemeral CareOnCloud Generic Interface ticket-create acceptance endpoint',
                    MappingInbound => {}, MappingOutbound => {}, Type => 'Ticket::TicketCreate',
                },
                TicketUpdate => {
                    Description => 'Ephemeral CareOnCloud Generic Interface acceptance endpoint',
...
my $After = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ScopeGet( TicketID => $TicketID );
...
my $CreatedScope = $Kernel::OM->Get('Kernel::System::CareOnCloud::TicketAudit')->ScopeGet( TicketID => $CreatedTicketID );
my $CreatedEvents = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->List(
...
my $CreatedArticleEvents = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->List(
...
my $CreatedVerify = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->Verify( Subject => $Subject, TenantID => $TenantID );
