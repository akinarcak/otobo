...
$RequiredFiles = @(
    ...
    'development/careoncloud/compose.yml',
    'development/careoncloud/.env.example',
    'development/careoncloud/Test-CleanPackageLifecycle.ps1',
    'development/careoncloud/Test-CareOnCloudBrand.ps1',
    'development/careoncloud/Test-CriticalLanguage.ps1',
    'development/careoncloud/Generate-CpanSbom.ps1',
    'development/careoncloud/Test-CpanSbom.ps1',
    'development/careoncloud/Generate-SourceArtifact.ps1',
    'development/careoncloud/Test-SourceArtifact.ps1',
    'development/careoncloud/migrate-careoncloud-brand.sh',
    'development/careoncloud/Accept-GenericInterfaceTicketUpdate.pl',
    'development/careoncloud/Accept-SchedulerTicketPendingCheck.pl',
    'development/careoncloud/Accept-GenericAgentTenantScope.pl',
...
if (-not $PackageSources) {
    throw 'No CareOnCloud package sources were found.'
}
...
Write-Host "Validated $($PackageSources.Count) CareOnCloud package manifests and file lists."
...
foreach ($StatusPackageSourceFile in $PackageSources) {
    ...
    $StatusBaseName = $PackageName -replace '^CareOnCloud', ''
...
[xml] $TicketAuditManifest = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/CareOnCloudTicketAudit.sopm') -Raw
$TicketAuditPackageVersion = $TicketAuditManifest.SelectSingleNode('/careoncloud_package/Version').InnerText
$TicketAuditStatusSource = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/Kernel/System/Console/Command/Admin/CareOnCloud/TicketAuditStatus.pm') -Raw
$TicketAuditStatusRegression = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/scripts/test/CareOnCloud/TicketAuditStatus.t') -Raw
foreach ($RequiredStatusVersionContract in @(
    "our `$VERSION = '$TicketAuditPackageVersion'",
    'Version => $VERSION',
    'CareOnCloudTicketAudit $VERSION:',
...
foreach ($RequiredRuntimeContract in @(
    ...
    'CareOnCloud_EXPECTED_GIT_COMMIT=$GitCommit',
...
foreach ($RequiredCleanPackage in @(
    ...
    if ($CleanLifecycleScript -notmatch [regex]::Escape("build_install $RequiredCleanPackage")) {
        throw "Clean package lifecycle does not install required CareOnCloud package: $RequiredCleanPackage"
    }
}
if ($CleanLifecycleScript -notmatch [regex]::Escape("grep -qx 18")) {
    throw 'Clean package lifecycle must require all 18 CareOnCloud package deployments.'
}
foreach ($RequiredCleanTestPackage in @(
    ...
    if ($CleanLifecycleScript -notmatch [regex]::Escape($RequiredCleanTestPackage)) {
        throw "Clean package lifecycle does not run required CareOnCloud test package: $RequiredCleanTestPackage"
    }
}
...
$TicketAuditWrapper = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/Kernel/System/Ticket/CareOnCloudAuditCustom.pm') -Raw
$TicketAuditService = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/Kernel/System/CareOnCloud/TicketAudit.pm') -Raw
$TicketAuditRegression = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/scripts/test/CareOnCloud/TicketAudit.t') -Raw
[xml] $TicketAuditManifest = Get-Content (Join-Path $RepositoryRoot 'packages/CareOnCloudTicketAudit/CareOnCloudTicketAudit.sopm') -Raw
...
foreach ($TicketAuditSource in @($TicketAuditWrapper, $TicketAuditService)) {
    if ($TicketAuditSource -notmatch [regex]::Escape("our `$VERSION = '$TicketAuditVersion';")) {
        throw "CareOnCloudTicketAudit source version does not match manifest version $TicketAuditVersion."
    }
}
...
foreach ($RequiredGenericAgentTenantContract in @(
    ...
    'AutomationScopeRun',
    "SELECT key_name FROM careoncloud_tenant WHERE status = 'active' ORDER BY key_name"
)) {
...
if ($ComposeText -notmatch '\$\{CareOnCloud_BIND_ADDRESS:-127\.0\.0\.1\}') {
    throw 'Compose must default its HTTP bind address to localhost.'
}
$ForbiddenPatterns = @(
    '(?im)^\s*(?:MYSQL_ROOT_PASSWORD|CareOnCloud_DB_ROOT_PASSWORD)\s*:\s*(?!\$\{)\S+',
...
$FoundationWorkflow = Get-Content (Join-Path $RepositoryRoot '.github/workflows/careoncloud-foundation.yml') -Raw
...
if ($RequireDocker) {
    ...
    $TemporaryEnvironment = Join-Path ([System.IO.Path]::GetTempPath()) "careoncloud-compose-$PID.env"
    try {
        @(
            'CareOnCloud_DB_ROOT_PASSWORD=ci-validation-only-password',
            'CareOnCloud_HTTP_PORT=8080'
...
Write-Host 'CareOnCloud ESM foundation checks passed.'
