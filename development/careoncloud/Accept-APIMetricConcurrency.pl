...
if ( ( $ARGV[0] // q{} ) eq '--worker' ) {
    ...
    my $Result = $Kernel::OM->Get('Kernel::System::CareOnCloud::APIMetric')->Record(
        TenantID => $TenantID,
        Route => $Route,
        Method => 'GET',
        StatusCode => 200,
...
$DB->Do(
    SQL => 'DELETE FROM careoncloud_api_metric WHERE tenant_id = ? AND window_start = ? AND route_key = ? AND method_name = ? AND status_code = ? AND error_code = ?',
    Bind => \@DimensionBind,
...
$DB->Prepare(
    SQL => 'SELECT COUNT(*), COALESCE(SUM(request_count),0), COALESCE(SUM(duration_sum_ms),0), COALESCE(MAX(duration_max_ms),0) FROM careoncloud_api_metric WHERE tenant_id = ? AND window_start = ? AND route_key = ? AND method_name = ? AND status_code = ? AND error_code = ?',
    Bind => \@DimensionBind,
);
