...
my $URL = shift // 'http://127.0.0.1:5000/careoncloud/public.pl?Action=PublicCareOnCloudMetrics';
my $Token = $ENV{CareOnCloud_METRICS_TOKEN} // q{};
die "CareOnCloud_METRICS_TOKEN must contain 16-256 non-whitespace characters\n"
...
my @Series = $Correct->{content} =~ m{^(careoncloud_[a-z0-9_]+)[ ][-+]?[0-9]+(?:\.[0-9]+)?$}gmx;
die "Expected 20 metric series, received " . scalar(@Series) . "\n" if @Series != 20;
my %Series = map { $_ => 1 } @Series;
die "Required health series are missing\n"
    if !$Series{careoncloud_up} || !$Series{careoncloud_tenants_active} || !$Series{careoncloud_observability_query_errors};
say 'CareOnCloud observability HTTP acceptance: PASS (401/401/200, 20 fixed series)';
