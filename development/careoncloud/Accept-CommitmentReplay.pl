...
$DB->Prepare(
    SQL => 'SELECT e.commitment_id, e.id FROM careoncloud_commitment_event e INNER JOIN careoncloud_commitment_instance i ON i.id = e.commitment_id AND i.tenant_id = e.tenant_id WHERE e.tenant_id = ? ORDER BY e.id DESC',
...
die "Acceptance outbox insert failed\n" if !$DB->Do(
    SQL => 'INSERT INTO careoncloud_escalation_outbox (tenant_id, commitment_id, commitment_event_id, action_key, action_type, payload_json, status, attempt_count, lifetime_attempt_count, replay_count, available_time, last_error, create_time, change_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp, current_timestamp)',
    Bind => \@Bind,
);
$DB->Prepare(
...
$DB->Prepare(
    SQL => 'SELECT status, attempt_count, lifetime_attempt_count, replay_count, delivery_ref, response_code FROM careoncloud_escalation_outbox WHERE tenant_id = ? AND id = ?',
...
my $Audit = $Kernel::OM->Get('Kernel::System::CareOnCloud::Audit')->List(
