...
my$UserID=47;my$Directory=$Kernel::OM->Get('Kernel::System::CareOnCloud::TenantDirectory');my$Change=$Kernel::OM->Get('Kernel::System::CareOnCloud::Change');my$Ctx=$Directory->ContextGet(UserID=>$UserID);die"User context unavailable\n"if!$Ctx->{Success};my$Subject=$Ctx->{Subject};
my@Seed=(
