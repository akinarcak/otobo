...
sub TenantExists {
    my ($ID) = @_;
    $DB->Prepare( SQL => 'SELECT 1 FROM d724_tenant WHERE key_name = ?', Bind => [ \$ID ], Limit => 1 );
...
for my $T (@Tenant) {
    ...
    $DB->Prepare( SQL => 'SELECT status, COUNT(*) FROM d724_request WHERE tenant_id = ? GROUP BY status ORDER BY status', Bind => [ \$T->{ID} ] );
    my %Status;
