...
if (!$TicketID) {
    $TicketID = $Ticket->TicketCreate(
        TN => $TN, Title => 'D724 atomic ticket and article audit acceptance',
    ...
    my $ArticleID = $Kernel::OM->Get('Kernel::System::Ticket::Article')->BackendForChannel( ChannelName => 'Internal' )->ArticleCreate(
        ...
        From => 'D724 Demo Agent <demo.agent@example.com>', To => 'D724 Demo Customer <demo.customer@example.com>',
        Subject => 'Atomic article audit accepted', Body => 'This visible demo article proves the audited MIME article flow.',
        ContentType => 'text/plain; charset=utf-8', HistoryType => 'AddNote', HistoryComment => 'D724 ticket audit acceptance',
